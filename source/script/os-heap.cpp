...
int32_t OSHeapManager::ceilPowerOfTwo(int32_t x)
{
	x--;
	x |= x / 2;
	x |= x >> 2;
	...
	if(first){
		...
		if(free_page_size < size){
			if(free_page_size > ((ALIGN + ALIGN - 1 + sizeof(SmallBlock) + OS_DUMMY_ID_SIZE) & ~(ALIGN-1))){
				...
				*(int32_t*)p = DUMMY_SMALL_FREE_ID_PRE;
				*(int32_t*)(p + small_block->getDataSize() + OS_SIZEOF_INT) = DUMMY_SMALL_FREE_ID_POST;
#endif
...
#ifdef OS_DEBUG
	*(int32_t*)p = DUMMY_SMALL_USED_ID_PRE;
	p += OS_SIZEOF_INT;
	*(int32_t*)(p + data_size) = DUMMY_SMALL_USED_ID_POST;
#endif

	OS_ASSERT("Heap align corrupted!" && ((int32_t)(intptr_t)(p) % ALIGN) == 0);
...
void OSHeapManager::freeSmall(void * p)
{
	...
	OS_ASSERT("Heap corrupted or trying to free alien memory" && *(int32_t*)p == DUMMY_SMALL_USED_ID_PRE);
	*(int32_t*)p = DUMMY_SMALL_FREE_ID_PRE;
#endif
...
#ifdef OS_DEBUG
	{
		int32_t * check_p = (int32_t*)((OS_BYTE*)p + small_block->getDataSize() + OS_SIZEOF_INT);
		OS_ASSERT("Heap corrupted!" && *check_p == DUMMY_SMALL_USED_ID_POST);
		*check_p = DUMMY_SMALL_FREE_ID_POST;
		// OS_MEMSET((int32_t*)p+1, DEAD_BYTE, small_block->getDataSize());
...
#ifdef OS_DEBUG
	p = (OS_BYTE*)p - OS_SIZEOF_INT;
	OS_ASSERT("Heap corrupted!" && *(int32_t*)p == DUMMY_SMALL_USED_ID_PRE);
#endif

	SmallBlock * small_block = (SmallBlock*)OS_STRUCT_OFFS((SmallBlock*)p, -1);

#ifdef OS_DEBUG
	OS_ASSERT("Heap corrupted!" && *(int32_t*)((OS_BYTE*)p + small_block->getDataSize() + OS_SIZEOF_INT) == DUMMY_SMALL_USED_ID_POST);
#endif
...
#ifdef OS_DEBUG
		*(int32_t*)(OS_STRUCT_OFFS(block, 1)) = DUMMY_MEDIUM_FREE_ID_PRE;
		*(int32_t*)((OS_BYTE*)(OS_STRUCT_OFFS(block, 1)) + block->getDataSize() + OS_SIZEOF_INT) = DUMMY_MEDIUM_FREE_ID_POST;
#endif
...
#ifdef OS_DEBUG
	*(int32_t*)p = DUMMY_MEDIUM_USED_ID_PRE;
	p += OS_SIZEOF_INT;
	*(int32_t*)(p + data_size) = DUMMY_MEDIUM_USED_ID_POST;
#endif
...
void OSHeapManager::freeMedium(void * p)
{
...
#ifdef OS_DEBUG
	p = (OS_BYTE*)p - OS_SIZEOF_INT;
	OS_ASSERT("Heap corrupted!" && *(int32_t*)p == DUMMY_MEDIUM_USED_ID_PRE);
	// *(int32_t*)p = DUMMY_MEDIUM_FREE_ID_PRE;
#endif
...
#ifdef OS_DEBUG
	{
		OS_ASSERT("Heap corrupted!" && *((int32_t*)((OS_BYTE*)p + block->getDataSize() + OS_SIZEOF_INT))	== DUMMY_MEDIUM_USED_ID_POST);
	...
	*(int32_t*)(OS_STRUCT_OFFS(block, 1)) = DUMMY_MEDIUM_FREE_ID_PRE;
	*(int32_t*)((OS_BYTE*)(OS_STRUCT_OFFS(block, 1)) + block->getDataSize() + OS_SIZEOF_INT) = DUMMY_MEDIUM_FREE_ID_POST;
	// OS_MEMSET((int32_t*)p+1, DEAD_BYTE, block->getDataSize());
#endif
...
#ifdef OS_DEBUG
	*(int32_t*)p = DUMMY_LARGE_USED_ID_PRE;
	p += OS_SIZEOF_INT;
	*(int32_t*)(p + data_size) = DUMMY_LARGE_USED_ID_POST;
#endif
...
#ifdef OS_DEBUG
	p = (OS_BYTE*)p - OS_SIZEOF_INT;
	OS_ASSERT("Heap corrupted!" && *(int32_t*)p == DUMMY_LARGE_USED_ID_PRE);
	*(int32_t*)p = DUMMY_LARGE_FREE_ID_PRE;
#endif
...
#ifdef OS_DEBUG
	{
		int32_t * check_p = (int32_t*)((OS_BYTE*)p + block->getDataSize() + OS_SIZEOF_INT);
...
#ifdef OS_DEBUG
	OS_ASSERT("Heap corrupted!" && *(int32_t*)((OS_BYTE*)p + block->getDataSize() + OS_SIZEOF_INT) == DUMMY_LARGE_USED_ID_POST);
#endif
...
void * OSHeapManager::malloc(int32_t size OS_DBG_FILEPOS_DECL)
{
#ifdef OS_DEBUG
	cur_id++;
	/* if(cur_id == 1362){
...
#ifndef USE_STD_MALLOC
#ifndef OS_USE_HEAP_SAVING_MODE
	switch(((OS_BYTE*)p)[-1-(int32_t)OS_DUMMY_ID_SIZE/2]){
	case BT_SMALL:
		freeSmall(p);
		break;
...
#else
	if(((OS_BYTE*)p)[-1-(int32_t)OS_DUMMY_ID_SIZE/2] & BLOCK_TYPE_MASK){
		freeSmall(p);
	}else{
		freeLarge(p);
	}
...
void OSHeapManager::setBreakpointId(int32_t id)
{
}

int32_t OSHeapManager::getAllocatedBytes()
{
#ifndef OS_USE_HEAP_SAVING_MODE
...
int32_t OSHeapManager::getMaxAllocatedBytes()
{
	OS_ASSERT(false);
	return 0;
}

int32_t OSHeapManager::getUsedBytes()
{
#ifndef OS_USE_HEAP_SAVING_MODE
...
int32_t OSHeapManager::getCachedBytes()
{
...
void OSHeapManager::checkMemory()
{
		...
		small_block != &dummy_small_block; small_block = small_block->next)
	{
		...
		OS_ASSERT("Heap corrupted!" && *(int32_t *)(OS_STRUCT_OFFS(small_block, 1)) == DUMMY_SMALL_USED_ID_PRE);
		OS_ASSERT("Heap corrupted!" && *(int32_t *)((OS_BYTE *)(OS_STRUCT_OFFS(small_block, 1)) + block_data_size + OS_SIZEOF_INT) == DUMMY_SMALL_USED_ID_POST);
	...
	for(int32_t i = 0; i < SMALL_SLOT_COUNT; i++){
			...
			free_small_block; free_small_block = free_small_block->next)
		{
			...
			OS_ASSERT("Heap corrupted!" && *(int32_t *)(OS_STRUCT_OFFS(small_block, 1)) == DUMMY_SMALL_FREE_ID_PRE);
			OS_ASSERT("Heap corrupted!" && *(int32_t *)((OS_BYTE *)(OS_STRUCT_OFFS(small_block, 1)) + small_block->getDataSize() + OS_SIZEOF_INT) == DUMMY_SMALL_FREE_ID_POST);
		}
	}
		...
		block = block->next)
	{
		...
		if(!block->is_free){
			...
			OS_ASSERT("Heap corrupted!" && *(int32_t *)(OS_STRUCT_OFFS(block, 1)) == DUMMY_MEDIUM_USED_ID_PRE);
			OS_ASSERT("Heap corrupted!" && *(int32_t *)((OS_BYTE *)(OS_STRUCT_OFFS(block, 1)) + block_data_size + OS_SIZEOF_INT) == DUMMY_MEDIUM_USED_ID_POST);

			used_size += block->size;
			data_size += block_data_size;
		}else{
			// OS_ASSERT("Heap corrupted!" && data_size < page_size);
			OS_ASSERT("Heap corrupted!" && *(int32_t *)(OS_STRUCT_OFFS(block, 1)) == DUMMY_MEDIUM_FREE_ID_PRE);
			OS_ASSERT("Heap corrupted!" && *(int32_t *)((OS_BYTE *)(OS_STRUCT_OFFS(block, 1)) + block_data_size + OS_SIZEOF_INT) == DUMMY_MEDIUM_FREE_ID_POST);
		}
...
{
	int32_t len = (int32_t)OS_STRLEN(buf);
...
void OSHeapManager::writeBlockHeader(OS * os, OS::FileHandle * f, int32_t type)
{
	char buf[256];
#ifdef OS_DEBUG
