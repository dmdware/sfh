...
#define OS_PTR_HASH(p) ((int32_t)(intptr_t)(p) >> 2)
...
#define getarg(i,pos,size)		(int32_t)(((i)>>pos) & OS_MASK1(size, 0))
#define setarg(i,v,pos,size)	((i) = (int32_t)(((i)&OS_MASK0(size,pos)) | (((Instruction)(v))<<pos) & OS_MASK1(size, pos)))
...
#define OS_SETARG_sBx(i,b)	OS_SETARG_Bx((i),((uint32_t)((b)+OS_MAXARG_sBx)))
...
static void checkFlagClear(int32_t instruction, int32_t flag)
{
...
static int32_t OS_VSNPRINTF(OS_CHAR * str, size_t size, const OS_CHAR *format, va_list va)
{
...
static int32_t OS_SNPRINTF(OS_CHAR * str, size_t size, const OS_CHAR *format, ...)
{
	va_list va;
	va_start(va, format);
	int32_t ret = OS_VSNPRINTF(str, size, format, va);
	va_end(va);
	return ret;
}
...
template <> int32_t OS_getMaxValue<int32_t>(){ return INT_MAX; }
...
static inline long double toLittleEndianByteOrder(long double val)
{
	...
	long double r;
	for(int32_t i = 0; i < (int32_t)sizeof(long double); i++){
...
template <class T>
static bool parseSimpleRadix(const OS_CHAR *& p_str, T& p_val, int32_t radix)
{
	T val = 0, prev_val = 0;
	...
	for(int32_t cur;; str++){
		if(*str >= OS_TEXT('0') && *str <= OS_TEXT('9')){
			cur = (int32_t)(*str - OS_TEXT('0'));
		}else if(*str >= OS_TEXT('a') && *str <= (OS_TEXT('z'))){
			cur = 10 + (int32_t)(*str - OS_TEXT('a'));
		}else if(*str >= OS_TEXT('A') && *str <= OS_TEXT('Z')){
			cur = 10 + (int32_t)(*str - OS_TEXT('A'));
		}else{
			break;
...
bool OS::Utils::parseFloat(const OS_CHAR *& str, OS_FLOAT& result, ENumberParseType parse_type, int32_t int_radix)
{
	...
	if(*str == OS_TEXT('.')){ // parse float
		...
		if(sign == 1 && start_str+1 == str && *start_str == OS_TEXT('1') && str[1] == OS_TEXT('#')){
			...
			int32_t i = 0;
			for(; spec[i]; i++){
				int32_t spec_len = (int32_t)OS_STRLEN(spec[i]);
...
{
	OS_SNPRINTF(dst, sizeof(OS_CHAR)*63, OS_TEXT("%li"), (int64_t)a);
	return dst;
}
...
double OS::Utils::round(double a, int32_t precision)
{
	if(precision <= 0){
		if(precision < 0){
			double p = 10.0;
...
OS_CHAR * OS::Utils::numToStr(OS_CHAR * dst, OS_FLOAT a, int32_t precision)
{
	if(precision <= 0) {
		if(precision < 0) {
	...
	int32_t n = OS_SNPRINTF(dst, sizeof(OS_CHAR)*127, FloatFormatStr<OS_NUMBER>::precisionFmt(), precision, a);
...
int32_t OS::Utils::keyToHash(const void * p_buf, int32_t size)
{
	OS_ASSERT(size >= 0);
	if(size < 0) size = 0;
	int32_t step = (size >> OS_STR_HASH_LIMIT_SHIFT) + 1;
	const OS_BYTE * buf = (const OS_BYTE*)p_buf;
	const OS_BYTE * end = buf + size;
	int32_t hash = OS_STR_HASH_START_VALUE;
...
int32_t OS::Utils::keyToHash(const void * buf1, int32_t size1, const void * buf2, int32_t size2)
{
...
int32_t OS::Utils::cmp(const void * buf1, int32_t len1, const void * buf2, int32_t len2)
{
	int32_t len = len1 < len2 ? len1 : len2;
	int32_t cmp = OS_MEMCMP(buf1, buf2, len);
...
OS::Core::String::String(OS * os, const OS_CHAR * str, int32_t len)
{
	allocator = os;
...
OS::Core::String::String(OS * os, const OS_CHAR * str, int32_t len, const OS_CHAR * str2, int32_t len2)
{
	allocator = os;
...
OS::Core::String::String(OS * os, const OS_CHAR * str, int32_t len, bool trim_left, bool trim_right)
{
	allocator = os;
...
OS::Core::String::String(OS * os, const void * buf, int32_t size)
{
	allocator = os;
...
OS::Core::String::String(OS * os, const void * buf1, int32_t size1, const void * buf2, int32_t size2)
{
	allocator = os;
...
OS::Core::String::String(OS * os, const void * buf1, int32_t size1, const void * buf2, int32_t size2, const void * buf3, int32_t size3)
{
	allocator = os;
...
OS::Core::String::String(OS * os, OS_FLOAT value, int32_t precision)
{
	allocator = os;
...
OS::Core::String OS::Core::String::format(OS * allocator, int32_t temp_buf_len, const OS_CHAR * fmt, ...)
{
...
OS::Core::String OS::Core::String::formatVa(OS * allocator, int32_t temp_buf_len, const OS_CHAR * fmt, va_list va)
{
	SaveStackSize saveStackSize(allocator); (void)saveStackSize;
	GCStringValue * string = allocator->core->pushStringValueVa(temp_buf_len, fmt, va);
	return String(allocator, string);
...
int32_t OS::Core::String::cmp(const String& b) const
{
	if(string == b.string){
		return 0;
	}
...
int32_t OS::Core::String::cmp(const OS_CHAR * b) const
{
	return Utils::cmp(string->toChar(), string->data_size, b, (int32_t)OS_STRLEN(b));
}

int32_t OS::Core::String::getHash() const
{
	return string->hash;
}
...
OS_NUMBER OS::Core::String::toNumberRadix(int32_t radix) const
{
...
{
	return append(str, (int32_t)OS_STRLEN(str));
}

OS::Core::Buffer& OS::Core::Buffer::append(const OS_CHAR * str, int32_t len)
{
	return append((const void*)str, len * sizeof(OS_CHAR));
}

OS::Core::Buffer& OS::Core::Buffer::append(const void * buf, int32_t size)
{
	freeCacheStr();
...
void OS::Core::Buffer::swap(Buffer& b)
{
	...
	int32_t temp_capacity = buffer.capacity;
	int32_t temp_count = buffer.count;
	int32_t temp_pos = pos;
...
OS::String::String(OS * allocator, const OS_CHAR * str1, int32_t len1, const OS_CHAR * str2, int32_t len2): super(allocator, str1, len1, str2, len2)
{
	allocator->retain();
}

OS::String::String(OS * allocator, const OS_CHAR * str, int32_t len): super(allocator, str, len)
{
	allocator->retain();
}

OS::String::String(OS * allocator, const OS_CHAR * str, int32_t len, bool trim_left, bool trim_right): super(allocator, str, len, trim_left, trim_right)
{
	allocator->retain();
}

OS::String::String(OS * allocator, const void * buf, int32_t size): super(allocator, buf, size)
{
	allocator->retain();
}

OS::String::String(OS * allocator, const void * buf1, int32_t size1, const void * buf2, int32_t size2): super(allocator, buf1, size1, buf2, size2)
{
	allocator->retain();
}
...
OS::String::String(OS * allocator, OS_FLOAT value, int32_t precision): super(allocator, value, precision)
{
	allocator->retain();
}
...
{
	SaveStackSize saveStackSize(allocator); (void)saveStackSize;
	Core::GCStringValue * string = allocator->core->pushStringValue(toChar(), getDataSize(), str, (int32_t)OS_STRLEN(str)*sizeof(OS_CHAR));
	return *this = String(allocator, string);
}
...
OS::Core::Tokenizer::TokenData::TokenData(TextData * p_text_data, const String& p_str, TokenType p_type, int32_t p_line, int32_t p_pos): str(p_str)
{
...
const int32_t OS::Core::Tokenizer::operator_count = sizeof(operator_desc) / sizeof(operator_desc[0]);

int32_t OS::Core::Tokenizer::compareOperatorDesc(const void * a, const void * b) 
{
	const OperatorDesc * op0 = (const OperatorDesc*)a;
	const OperatorDesc * op1 = (const OperatorDesc*)b;
	// return (int32_t)OS_STRLEN(op1->name) - (int32_t)OS_STRLEN(op0->name);
...
OS::Core::Tokenizer::TokenData * OS::Core::Tokenizer::removeToken(int32_t i)
{
...
void OS::Core::Tokenizer::insertToken(int32_t i, TokenData * token OS_DBG_FILEPOS_DECL)
{
	getAllocator()->vectorInsertAtIndex(tokens, i, token OS_DBG_FILEPOS_PARAM);
}

bool OS::Core::Tokenizer::parseText(const OS_CHAR * text, int32_t len, const String& filename, bool is_real_file, OS_ESourceCodeType source_code_type, bool check_utf8_bom)
{
	...
	while(str < str_end)
	{
		...
		allocator->vectorAddItem(text_data->lines, String(allocator, str, (int32_t)(line_end - str)) OS_DBG_FILEPOS);
		str = line_end+1;
#endif
	}
...
OS::Core::Tokenizer::TokenData * OS::Core::Tokenizer::addToken(const String& str, TokenType type, int32_t line, int32_t pos OS_DBG_FILEPOS_DECL)
{
	OS * allocator = getAllocator();
...
bool OS::Core::Tokenizer::parseLines(OS_ESourceCodeType source_code_type, bool check_utf8_bom)
{
	...
	int32_t var_in_string = 0;
	
	const char * multi_string_id = NULL;
	int32_t multi_string_id_len = 0;
		...
		for(;;){
			if(template_enabled && is_template){
				...
				for(;;){
					...
					if(open_os_tag){
						buf.append(line_pos, (int32_t)(open_os_tag - line_pos));
						if(buf.getSize() > 0){
							addToken(buf, OUTPUT_STRING, cur_line, (int32_t)(str - line_start) OS_DBG_FILEPOS);
						}
						str = open_os_tag + 2;
						is_template = false;

						if(str[0] == OS_TEXT('=')){
							addToken(String(allocator, str, 1), OUTPUT_NEXT_VALUE, cur_line, (int32_t)(str - line_pos) OS_DBG_FILEPOS);
							str++;
						}
						break;
					}
					...
					if(cur_line+1 >= text_data->lines.count){
						if(buf.getSize() > 0){
							addToken(buf, OUTPUT_STRING, cur_line, (int32_t)(str - line_pos) OS_DBG_FILEPOS);
						}
						return true;
					}
			...
			if(var_in_string == 1 && *str == OS_TEXT('}')){
				...
				addToken(String(allocator, OS_TEXT(")")), END_BRACKET_BLOCK, cur_line, (int32_t)(str - line_start) OS_DBG_FILEPOS);
				addToken(String(allocator, OS_TEXT("}")), AFTER_INJECT_VAR, cur_line, (int32_t)(str - line_start) OS_DBG_FILEPOS);
				...
				str++;
				addToken(String(allocator, token_start+1, str-token_start-2), STRING, cur_line, (int32_t)(token_start - line_start) OS_DBG_FILEPOS);
				continue;
			}else if(*str == OS_TEXT('"')){
				string_type = QUOTE;
			}else if(*str == OS_TEXT('\'')){
				...
				if(var_in_string){
					error = ERROR_SYNTAX;
					cur_pos = (int32_t)(str - line_start);
					return false;
				}
				str += 3;
				multi_string_id = str;
				while(*str && *str != OS_TEXT('\'') && *str != OS_TEXT('"')) str++;
				if(!*str || (int32_t)(str - multi_string_id) < 2){
					cur_pos = (int32_t)(str - line_start);
					error = ERROR_CONST_STRING;
					return false;
				}
				...
				if(*str){
					...
					if(cur_line+1 >= text_data->lines.count){
						cur_pos = (int32_t)(str + OS_STRLEN(str) - line_start);
						error = ERROR_CONST_STRING;
						return false;
					}
					str = line_start = text_data->lines[++cur_line].toChar();
				...
				for(;;){
					...
					while(*str){
						if(OS_MEMCMP(str, multi_string_id, multi_string_id_len) == 0){
							...
							if(start && buf.buffer.count >= (int32_t)sizeof(OS_CHAR)){
							...
							addToken(buf, STRING, cur_line, (int32_t)(token_start - line_start) OS_DBG_FILEPOS);
							finished = true;
							break;
						}
						OS_CHAR c = *str++;
						if(string_type == MULTI_QUOTE){
							if(c == OS_TEXT('$') && string_type != SIMPLE && *str == OS_TEXT('{')){
								...
								str++;
								addToken(buf, STRING, cur_line, (int32_t)(token_start - line_start) OS_DBG_FILEPOS);
								addToken(String(allocator, OS_TEXT("${")), BEFORE_INJECT_VAR, cur_line, (int32_t)(str - 2 - line_start) OS_DBG_FILEPOS);
								addToken(String(allocator, OS_TEXT("(")), BEGIN_BRACKET_BLOCK, cur_line, (int32_t)(str - 2 - line_start) OS_DBG_FILEPOS);
								break;
							}else if(c == OS_TEXT('\\')){
								switch(*str){
					...
					if(c == OS_TEXT('$') && string_type != SIMPLE && *str == OS_TEXT('{')){
						...
						if(string_type == SIMPLE){
							...
							default:
								{
									int32_t val;
									int32_t max_val = sizeof(OS_CHAR) == 2 ? 0xFFFF : 0xFF;
			...
			if(*str == OS_TEXT('/')){
				if(str[1] == OS_TEXT('/')){ // begin line comment
					if(settings.save_comments){
						addToken(String(allocator, str), COMMENT_LINE, cur_line, (int32_t)(str - line_start) OS_DBG_FILEPOS);
					}
					break;
				}
				if(str[1] == OS_TEXT('*')){ // begin multi line comment
					...
					int32_t startLine = cur_line;
					int32_t startPos = (int32_t)(str - line_start);
					for(str += 2;;){
						const OS_CHAR * end = OS_STRSTR(str, OS_TEXT("*/"));
						if(end){
							if(settings.save_comments){
								comment.append(str, (int32_t)(end+2 - str));
			...
			{ // parse name
				...
				String name = String(allocator, name_start, (int32_t)(str - name_start));
				addToken(name, NAME, cur_line, (int32_t)(name_start - line_start) OS_DBG_FILEPOS);
				continue;
			}
			// parse operator
			if(0 && (*str == OS_TEXT('-') || *str == OS_TEXT('+')) && (str[1] >= OS_TEXT('0') && str[1] <= OS_TEXT('9'))){
				...
				for(i = 0; i < operator_count; i++){
					...
					int32_t len = operator_desc[i].len;
					if(OS_STRNCMP(str, operator_desc[i].name, len) == 0){
						addToken(String(allocator, str, (int32_t)len), operator_desc[i].type, cur_line, (int32_t)(str - line_start) OS_DBG_FILEPOS);
			...
			}
			{
				...
				if(parseFloat(str, fval, true)){
					TokenData * token = addToken(String(allocator, token_start, (int32_t)(str - token_start), false, true), 
						NUMBER, cur_line, (int32_t)(token_start - line_start) OS_DBG_FILEPOS);
					//sfh edit
					//token->setFloat(fval);
					token->setInt(fval);
					continue;
...
OS::Core::Compiler::Expression * OS::Core::Compiler::ExpressionList::insert(int32_t i, Expression * exp OS_DBG_FILEPOS_DECL)
{
	allocator->vectorInsertAtIndex(*this, i, exp OS_DBG_FILEPOS_PARAM);
	return exp;
}

OS::Core::Compiler::Expression * OS::Core::Compiler::ExpressionList::removeIndex(int32_t i)
{
...
void OS::Core::Compiler::ExpressionList::swap(ExpressionList& list)
{
	...
	int32_t save_count = count;
	int32_t save_capacity = capacity;
...
OS::Core::String OS::Core::Compiler::Expression::getSlotStr(OS::Core::Compiler * compiler, Scope * scope, int32_t slot_num, int32_t up_count)
{
	struct Lib {
	...
	for(;;){
		for(int32_t i = scope->locals.count-1; i >= 0; i--){
...
void OS::Core::Compiler::Expression::debugPrint(Buffer& out, OS::Core::Compiler * compiler, Scope * scope, int32_t depth)
{
	OS * allocator = getAllocator();
...
int32_t OS::Core::Compiler::cacheString(Table * strings_table, Vector<String>& strings, const String& str)
{
	Value index(str);
	Property * prop = strings_table->get(index, OS_VALUE_TYPE(index));
	if(prop){
		OS_ASSERT(OS_IS_VALUE_NUMBER(prop->value));
		int32_t i; OS_NUMBER_TO_INT(i, OS_VALUE_NUMBER(prop->value));
		return i;
	}
...
int32_t OS::Core::Compiler::cacheString(const String& str)
{
	return cacheString(prog_strings_table, prog_strings, str);
}

int32_t OS::Core::Compiler::cacheNumber(OS_NUMBER num)
{
	Value index(num);
...
OS::Core::Compiler::DebugInfoItem::DebugInfoItem(int32_t p_line, int32_t p_pos)
{
	line = (OS_U32)p_line;
	pos = (OS_U32)p_pos;
}
...
void OS::Core::Compiler::writeDebugInfo(Expression * exp)
{
	if(allocator->core->settings.create_debug_info){
		int32_t opcode_pos = getOpcodePos();
...
void OS::Core::Compiler::writeJumpOpcode(int32_t offs)
{
	offs += 1;
...
void OS::Core::Compiler::fixJumpOpcode(int32_t offs, int32_t pos)
{
		...
		if(optimization_enabled && prog_opcodes.count - prog_optimize_offs >= 2){
			...
			if(OS_GET_OPCODE_TYPE(prev) == OP_MOVE){
				...
				if(OS_GET_OPCODE_TYPE(cur) == OP_MOVE){
					int32_t prev_a = OS_GETARG_A(prev);
					int32_t cur_a = OS_GETARG_A(cur);
					if(prev_a == OS_GETARG_B(cur) && cur_a == OS_GETARG_B(prev)){
						...
						*/
						...
						int32_t prev_b = OS_GETARG_B(prev & ~OS_OPCODE_CONST_B);
						OS_SETARG_B(instruction, prev_b);
						int32_t cur_b = OS_GETARG_B(cur & ~OS_OPCODE_CONST_B);
...
int32_t OS::Core::Compiler::getOpcodePos()
{
	return prog_opcodes.count;
}

int32_t OS::Core::Compiler::writeOpcode(OS_U32 opcode)
{
	int32_t i = prog_opcodes.count;
	allocator->vectorAddItem(prog_opcodes, opcode OS_DBG_FILEPOS);
	return i;
}

int32_t OS::Core::Compiler::writeOpcode(OpcodeType opcode)
{
	return writeOpcode(OS_FROM_OPCODE_TYPE(opcode));
}

int32_t OS::Core::Compiler::writeOpcodeABC(OpcodeType p_opcode, int32_t a, int32_t b, int32_t c)
{
	int32_t opcode = OS_FROM_OPCODE_TYPE(p_opcode);
...
int32_t OS::Core::Compiler::writeOpcodeABx(OpcodeType opcode, int32_t a, int32_t b)
{
	OS_ASSERT(a >= 0 && a <= OS_MAXARG_A);
...
void OS::Core::Compiler::writeOpcodeAt(OS_U32 opcode, int32_t pos)
{
	...
	case EXP_TYPE_FUNCTION:
		{
			...
			int32_t prog_func_index = scope->prog_func_index; // prog_functions.indexOf(scope);
			OS_ASSERT(prog_func_index >= 0);
			
			int32_t pos = writeOpcodeABC(OP_NEW_FUNCTION, exp->slots.a, prog_func_index, 0); (void)pos;
			...
			writeOpcodeABC(OP_RETURN, 0, 0, 1); // return std::list<Widget*>::iterator
	...
	case EXP_TYPE_FOR_LOOP_SCOPE:
		{
			Scope * scope = dynamic_cast<Scope*>(exp);
			OS_ASSERT(scope);
			int32_t start_code_pos = getOpcodePos(), start_for_post = 0;
			if(exp->type == EXP_TYPE_FOR_LOOP_SCOPE){
				for(int32_t i = 0; i < exp->list.count; i++){
	...
	case EXP_TYPE_QUESTION:
		{
			...
			int32_t if_jump_pos = writeOpcode(OP_JUMP);

			if(!writeOpcodes(scope, exp->list[1])){
				return false;
			}
			int32_t if_jump_to = prog_optimize_offs = getOpcodePos();
			if(exp->list.count == 3){ // && exp->list[2]->list.count > 0){
				int32_t jump_pos = writeOpcode(OP_JUMP);
	...
	case EXP_TYPE_TRY_CATCH:
		{
			...
			int32_t op_try_start = prog_opcodes.count;
			if(!writeOpcodes(scope, exp->list[0])){
				return false;
			}
			// writeDebugInfo(exp);
			int32_t op_jump_pos = writeOpcode(OP_JUMP);
			if(!writeOpcodes(scope, exp->list[1])){
				return false;
			}

			int32_t op_jump_to = getOpcodePos();
	...
	case EXP_TYPE_SWITCH_SCOPE:
		{
			Scope * scope = dynamic_cast<Scope*>(exp);
			OS_ASSERT(scope);
			int32_t start_code_pos = getOpcodePos();
			if(!writeOpcodes(scope, exp->list, true)){
				return false;
			}
			for(i = 0; i < scope->locals.count; i++){
			...
			for(int32_t i = 0; i < scope->case_labels.count; i++){
...
OS::Core::Compiler::Scope::LocalVar::LocalVar(const String& p_name, int32_t p_index, ECompiledValueType p_type): name(p_name)
{
	index = p_index;
	type = p_type;
	upvalue = false;
...
bool OS::Core::Compiler::Scope::addLoopBreak(int32_t pos, ELoopBreakType type)
{
	Scope * scope = this;
...
bool OS::Core::Compiler::Scope::addCaseLabel(TokenData * key, Expression * exp)
{
	...
	if(scope && type == EXP_TYPE_SWITCH_SCOPE){
#ifdef OS_DEBUG
		for(int32_t i = 0; i < case_labels.count; i++)
...
bool OS::Core::Compiler::Scope::setCaseLabelPos(TokenData * key, int32_t pos, Compiler* cmp)
{
	for(int32_t i = 0; i < case_labels.count; i++){
...
bool OS::Core::Compiler::Scope::setCaseLabelJump(TokenData * key, int32_t pos, Compiler* cmp)
{
	for(int32_t i = 0; i < case_labels.count; i++){
		SwitchCaseLabel& sw = case_labels[i];
		if(sw.key == key){
...
void OS::Core::Compiler::Scope::fixLoopBreaks(Compiler * compiler, int32_t scope_start_pos, int32_t scope_end_pos)
{
	for(int32_t i = 0; i < loop_breaks.count; i++){
		LoopBreak& loop_break = loop_breaks[i];
		if(loop_break.type == LOOP_BREAK){
			int32_t offs = scope_end_pos - loop_break.pos - 1;
			compiler->fixJumpOpcode(offs, loop_break.pos);
		}else{
			int32_t offs = scope_start_pos - loop_break.pos - 1;
			compiler->fixJumpOpcode(offs, loop_break.pos);
		}
	}
}
...
void OS::Core::Compiler::Scope::addTryBlock(int32_t start, int32_t end, Scope * catch_block)
{
...
int32_t OS::Core::Compiler::Scope::allocTempVar()
{
...
void OS::Core::Compiler::Scope::popTempVar(int32_t count)
{
...
void * OS::Core::Compiler::malloc(int32_t size OS_DBG_FILEPOS_DECL)
{
...
OS::Core::Tokenizer::TokenData * OS::Core::Compiler::setNextTokenIndex(int32_t i)
{
...
{
	OS_ASSERT(tokenizer);
	int32_t i, count = tokenizer->getNumTokens();
...
bool OS::Core::Compiler::isNextTokens(TokenType * list, int32_t count)
{
	bool ok = true;
	int32_t save_next_token_index = next_token_index;
...
OS::Core::Compiler::Expression * OS::Core::Compiler::expectExpressionValues(Expression * exp, int32_t ret_values, bool auto_no_values)
{
	...
	case EXP_TYPE_PARAMS:
		if(exp->ret_values > ret_values){
			for(int32_t i = exp->list.count-1; exp->ret_values > ret_values && i >= 0; i--){
	...
	while(exp->ret_values > ret_values){
		int32_t new_ret_values = exp->ret_values-1;
...
OS::Core::Compiler::Expression * OS::Core::Compiler::newExpressionFromList(ExpressionList& list, int32_t ret_values, bool auto_no_values)
{
	Expression * exp;
	if(list.count == 1){
		exp = list[0];
...
OS::Core::Compiler::Expression * OS::Core::Compiler::postCompilePass2(Scope * scope, Expression * exp)
{
		...
		if(exp->list.count == 0){
			...
			for(; scope; scope = scope->parent){
				for(int32_t i = scope->locals.count-1; i >= 0; i--){
					...
					bool found = false;
					for(int32_t j = 0; j < vars.count; j++){
	...
	case EXP_TYPE_PARAMS:
		{
			for(int32_t i = exp->list.count-1; i >= 0; i--){
...
OS::Core::Compiler::Expression * OS::Core::Compiler::postCompilePass3(Scope * scope, Expression * exp)
{
	struct Lib {
		...
		{
			for(int32_t i = 0; i < exp->list.count && !compiler->isError(); i++){
	...
	case EXP_TYPE_OBJECT_SET_BY_INDEX:
		...
		exp->slots.b = cacheNumber((OS_NUMBER)exp->token->getInt());
		break;
	...
	case EXP_TYPE_SET_DIM:
		{
			...
			for(int32_t i = 2; i < exp2->list.count; i++){
...
OS::Core::Compiler::Scope * OS::Core::Compiler::expectTextExpression()
{
	...
	int32_t ret_values = list.count == 1 && list[0]->ret_values > 0 && list[0]->type == EXP_TYPE_FUNCTION ? 1 : 0;
	if(!ret_values){
...
OS::Core::Compiler::Expression * OS::Core::Compiler::expectObjectOrFunctionExpression(Scope * scope, const Params& org_p, bool allow_finish_exp)
{
	...
	struct Lib {
		...
		void * malloc(int32_t size OS_DBG_FILEPOS_DECL)
		{
			return compiler->malloc(size OS_DBG_FILEPOS_PARAM);
		}

		Lib(Compiler * p_compiler, int32_t active_locals)
		{
			compiler = p_compiler;
	...
	for(readToken();;){
		...
		if(name_token->type == Tokenizer::BEGIN_ARRAY_BLOCK){
			...
			if(name_token->type == Tokenizer::OPERATOR_SUB){
				...
				name_token->setInt(-name_token->getInt());
			}else{
				name_token = recent_token;
			}
...
OS::Core::Compiler::Expression * OS::Core::Compiler::expectArrayExpression(Scope * scope, const Params& __p)
{
	...
	for(int32_t auto_index = 0;;){
		...
		index_token->setInt((OS_INT)(auto_index++));
...
OS::Core::Compiler::Expression * OS::Core::Compiler::expectParamsExpression(Scope * scope)
{
	struct Lib 
	{
		...
		{
			if(params->list.count > 1){
				for(int32_t i = 0; i < params->list.count; i++){
...
OS::Core::Compiler::Expression * OS::Core::Compiler::expectSwitchExpression(Scope * parent)
{
	...
	*/
	...
	for(int32_t i = 0; i < switch_scope->case_labels.count; i++){
...
OS::Core::Compiler::Expression * OS::Core::Compiler::expectForExpression(Scope * parent)
{
	...
	if(recent_token->type == Tokenizer::NAME && (exp->type == EXP_TYPE_PARAMS || exp->type == EXP_TYPE_NEW_LOCAL_VAR || exp->type == EXP_TYPE_NAME)){
...
#endif
		int32_t num_locals = vars.count;

		const int32_t temp_count = 2;
		...
		{
			Expression * params = new (malloc(sizeof(Expression) OS_DBG_FILEPOS)) Expression(EXP_TYPE_PARAMS, exp->token);
			for(int32_t i = num_locals; i < vars.count-1; i++){
		...
		*/
		list.add(loop_scope OS_DBG_FILEPOS);
		{
			...
			for(int32_t i = 0; i < num_locals+1; i++){
			...
			Expression * call_exp = new (malloc(sizeof(Expression) OS_DBG_FILEPOS)) Expression(EXP_TYPE_CALL, loop_scope->token);
			{
				...
				Expression * params = new (malloc(sizeof(Expression) OS_DBG_FILEPOS)) Expression(EXP_TYPE_PARAMS, loop_scope->token);
				for(int32_t i = num_locals+1; i < vars.count-1; i++){
					Expression * var_exp = vars[i];
					Expression * name_exp = new (malloc(sizeof(Expression) OS_DBG_FILEPOS)) Expression(EXP_TYPE_NAME, var_exp->token);
					OS_ASSERT(scope->function);
					name_exp->active_locals = scope->function->num_locals;
...
OS::Core::Compiler::Expression * OS::Core::Compiler::newBinaryExpression(Scope * scope, ExpressionType exp_type, TokenData * token, Expression * left_exp, Expression * right_exp)
{
	if(left_exp->isConstValue() && right_exp->isConstValue()){
		struct Lib {
			...
			Expression * newExpression(OS_INT val, Expression * left_exp, Expression * right_exp)
			{
				token = new (malloc(sizeof(TokenData) OS_DBG_FILEPOS)) TokenData(token->text_data, String(compiler->allocator, (OS_INT)val), Tokenizer::NUMBER, token->line, token->pos);
				token->setInt(val);
				Expression * exp = new (malloc(sizeof(Expression) OS_DBG_FILEPOS)) Expression(EXP_TYPE_CONST_NUMBER, token);
				exp->ret_values = 1;
				token->release();
	...
	case EXP_TYPE_ASSIGN:
		{
			...
			for(int32_t i = left_exp->list.count-1; i >= 0; i--){
...
bool OS::Core::Compiler::findLocalVar(LocalVarDesc& desc, Scope * scope, const String& name, int32_t active_locals, bool all_scopes, bool decl)
{
	OS_ASSERT(scope);
	for(int32_t up_count = 0, up_scope_count = 0;;){
		for(int32_t i = scope->locals.count-1; i >= 0; i--){
			const Scope::LocalVar& local_var = scope->locals[i];
...
void OS::Core::Compiler::registerUpvalue(Scope * scope, Expression * exp)
{
	...
	for(int32_t up_count = exp->local_var.up_count; up_count > 0; up_count--){
		OS_ASSERT(scope->function && scope->function->parent);
		scope = scope->function->parent;
	}
	for(;;){
	...
	for(int32_t up_count = exp->local_var.up_scope_count; up_count > 0; up_count--){
		OS_ASSERT(scope->parent);
		scope = scope->parent;
	}
#else
	OS_ASSERT(scope && exp && exp->local_var.up_count > 0);
	for(int32_t up_count = exp->local_var.up_count; up_count > 0;){
		OS_ASSERT(scope->parent);
		if(scope->type == EXP_TYPE_FUNCTION){
			up_count--;
		}
...
OS::Core::Compiler::Expression * OS::Core::Compiler::expectSingleExpression(Scope * scope, const Params& p)
{
	...
	case Tokenizer::OPERATOR_LOGIC_NOT:
		...
		case EXP_TYPE_CONST_FALSE:
			...
			case Tokenizer::OPERATOR_BIT_NOT:
				...
				exp->token->setInt((OS_INT)~exp->toInt());
				return finishValueExpressionNoAutoCall(scope, exp, p);
	...
	case Tokenizer::NAME:
		...
		if(token->str == allocator->core->strings->syntax_line){
			...
			token->setInt((OS_INT)(token->line + 1));
			exp = new (malloc(sizeof(Expression) OS_DBG_FILEPOS)) Expression(EXP_TYPE_CONST_NUMBER, token);
			exp->ret_values = 1;
			readToken();
			return finishValueExpressionNoNextCall(scope, exp, p);
...
void OS::Core::Compiler::debugPrintSourceLine(Buffer& out, TokenData * token)
{
	...
	if(recent_printed_line != token->line && token->line >= 0){
		...
		// line could be very int32_t so DON'T use format because of the format uses temp buffer of 10 Kb only
...
const OS_CHAR * OS::Core::Compiler::getExpName(ExpressionType type, ECompiledValueType val_type)
{
	...
	case EXP_TYPE_SET_PROPERTY_BY_LOCALS_AUTO_CREATE:
		return OS_TEXT("set property by locals std::list<Widget*>::iterator create");

	case EXP_TYPE_GET_SET_PROPERTY_BY_LOCALS_AUTO_CREATE:
		return OS_TEXT("get & set property by locals std::list<Widget*>::iterator create");
	...
	case EXP_TYPE_GET_PROPERTY_AUTO_CREATE:
		return OS_TEXT("get property std::list<Widget*>::iterator create");
	...
	case EXP_TYPE_GET_LOCAL_VAR_AUTO_CREATE:
		return OS_TEXT("get local var std::list<Widget*>::iterator create");

	case EXP_TYPE_GET_ENV_VAR:
		return OS_TEXT("get env var");

	case EXP_TYPE_GET_ENV_VAR_AUTO_CREATE:
		return OS_TEXT("get env var std::list<Widget*>::iterator create");
...
OS::Core::Program::~Program()
{
	...
	for(i = 0; i < num_strings; i++){
		int32_t j = i + num_numbers + CONST_STD_VALUES;
	...
	for(i = 0; i < num_functions; i++){
		...
		for(int32_t j = 0; j < func->num_locals; j++){
...
{
	writer->writeBytes(OS_COMPILED_HEADER, (int32_t)OS_STRLEN(OS_COMPILED_HEADER));

	int32_t i, len = (int32_t)OS_STRLEN(OS_VERSION)+1;
	...
	int32_t int_count = 0, float_count = 0, double_count = 0, long_double_count = 0;
	int32_t int_index = 0, float_index = 0, double_index = 0, long_double_index = 0;
	for(i = 0; i < prog_numbers.count; i++){
		long double val = prog_numbers[i];
		if(val >= 0 && (long double)(int32_t)val == val){
			int_count++;
			int_stream.writeUVariable(i - int_index); int_index = i;
			int_stream.writeUVariable((int32_t)val);
			continue;
		}
	...
	for(i = 0; i < prog_strings.count; i++){
		...
		int32_t data_size = str.getDataSize();
...
bool OS::Core::Program::loadFromStream(StreamReader * reader)
{
	...
	int32_t i, len = (int32_t)OS_STRLEN(OS_COMPILED_HEADER);
	if(!reader->checkBytes(OS_COMPILED_HEADER, len)){
		return false;
	}

	len = (int32_t)OS_STRLEN(OS_VERSION)+1;
	reader->movePos(1);
	if(!reader->checkBytes(OS_VERSION, len)){
		return false;
	}

	int32_t int_count = reader->readUVariable();
	int32_t float_count = reader->readUVariable();
	int32_t double_count = reader->readUVariable();
	int32_t long_double_count = reader->readUVariable();
	...
	int32_t opcodes_size = reader->readUVariable();
	int32_t num_debug_infos = reader->readUVariable();
	// OS_ASSERT(!num_debug_infos || num_debug_infos == opcodes_size || num_debug_infos + 1 == opcodes_size);
	int32_t prog_filename_string_index = reader->readUVariable();
	...
	Buffer buf(allocator);
	for(i = 0; i < num_strings; i++){
		int32_t data_size = reader->readUVariable();
	...
	for(i = 0; i < num_functions; i++){
		...
		for(j = 0; j < func->num_locals; j++){
			int32_t cached_name_index = reader->readUVariable();
	...
	for(i = 0; i < num_debug_infos; i++){
		int32_t line = reader->readUVariable();
		int32_t pos = reader->readUVariable();
...
OS::Core::Program::DebugInfoItem * OS::Core::Program::getDebugInfo(int32_t opcode_pos)
{
...
OS::Core::Program::DebugInfoItem::DebugInfoItem(int32_t p_line, int32_t p_pos)
{
	line = (OS_U32)p_line;
	pos = (OS_U32)p_pos;
}
...
void OS::Core::Program::pushStartFunction()
{
	...
	int32_t prog_func_index = OS_GETARG_B(opcode);
...
{
	int32_t size = reader->getSize() - reader->getPos();
	int32_t buf_size = 1024 * 128;
	void * buf = allocator->malloc(buf_size < size ? buf_size : size OS_DBG_FILEPOS);
	OS_ASSERT(buf || !size);
	for(; size > 0; size -= buf_size){
		int32_t chunk_size = buf_size <= size ? buf_size : size;
...
void OS::Core::StreamWriter::writeByte(int32_t value)
{
...
void OS::Core::StreamWriter::writeByteAtPos(int32_t value, int32_t pos)
{
	OS_ASSERT(value >= 0 && value <= 0xff);
	OS_BYTE le_value = toLittleEndianByteOrder((OS_BYTE)value);
...
void OS::Core::StreamWriter::writeUVariable(int32_t value)
{
	OS_ASSERT(value >= 0);
	for(;;){
		if(value >= 0x7f){
...
void OS::Core::StreamWriter::writeU16(int32_t value)
{
...
void OS::Core::StreamWriter::writeU16AtPos(int32_t value, int32_t pos)
{
	OS_ASSERT(value >= 0 && value <= 0xffff);
	OS_U16 le_value = toLittleEndianByteOrder((OS_U16)value);
	writeBytesAtPos(&le_value, sizeof(le_value), pos);
...
void OS::Core::StreamWriter::writeInt8(int32_t value)
{
	OS_ASSERT((int32_t)(OS_INT8)value == value);
	OS_INT8 le_value = toLittleEndianByteOrder((OS_INT8)value);
	writeBytes(&le_value, sizeof(le_value));
}

void OS::Core::StreamWriter::writeInt8AtPos(int32_t value, int32_t pos)
{
	OS_ASSERT((int32_t)(OS_INT8)value == value);
	OS_INT8 le_value = toLittleEndianByteOrder((OS_INT8)value);
	writeBytesAtPos(&le_value, sizeof(le_value), pos);
...
void OS::Core::StreamWriter::writeInt16(int32_t value)
{
	OS_ASSERT((int32_t)(OS_INT16)value == value);
	OS_INT16 le_value = toLittleEndianByteOrder((OS_INT16)value);
	writeBytes(&le_value, sizeof(le_value));
}

void OS::Core::StreamWriter::writeInt16AtPos(int32_t value, int32_t pos)
{
	OS_ASSERT((int32_t)(OS_INT16)value == value);
	OS_INT16 le_value = toLittleEndianByteOrder((OS_INT16)value);
	writeBytesAtPos(&le_value, sizeof(le_value), pos);
...
void OS::Core::StreamWriter::writeInt32(int32_t value)
{
	OS_INT32 le_value = toLittleEndianByteOrder((OS_INT32)value);
	writeBytes(&le_value, sizeof(le_value));
}

void OS::Core::StreamWriter::writeInt32AtPos(int32_t value, int32_t pos)
{
	OS_ASSERT((int32_t)(OS_INT32)value == value);
	OS_INT32 le_value = toLittleEndianByteOrder((OS_INT32)value);
	writeBytesAtPos(&le_value, sizeof(le_value), pos);
}
...
void OS::Core::StreamWriter::writeInt64AtPos(OS_INT64 value, int32_t pos)
{
	OS_INT64 le_value = toLittleEndianByteOrder((OS_INT64)value);
	writeBytesAtPos(&le_value, sizeof(le_value), pos);
}
...
void OS::Core::StreamWriter::writeFloatAtPos(float value, int32_t pos)
{
	float le_value = toLittleEndianByteOrder(value);
	writeBytesAtPos(&le_value, sizeof(le_value), pos);
}
...
void OS::Core::StreamWriter::writeDoubleAtPos(double value, int32_t pos)
{
	double le_value = toLittleEndianByteOrder(value);
	writeBytesAtPos(&le_value, sizeof(le_value), pos);
}
...
void OS::Core::StreamWriter::writeLongDoubleAtPos(long double value, int32_t pos)
{
	long double le_value = toLittleEndianByteOrder(value);
	writeBytesAtPos(&le_value, sizeof(le_value), pos);
}
...
int32_t OS::Core::MemStreamWriter::getPos() const
{
	return pos;
}

void OS::Core::MemStreamWriter::setPos(int32_t new_pos)
{
	OS_ASSERT(new_pos >= 0 && new_pos <= buffer.count);
	pos = new_pos;
}

int32_t OS::Core::MemStreamWriter::getSize() const
{
	return buffer.count;
}
...
void OS::Core::MemStreamWriter::reserveCapacity(int32_t new_capacity)
{
...
void OS::Core::MemStreamWriter::writeBytes(const void * buf, int32_t len)
{
...
void OS::Core::MemStreamWriter::writeBytesAtPos(const void * buf, int32_t len, int32_t pos)
{
	OS_ASSERT(pos >= 0 && pos <= buffer.count-len);
	OS_MEMCPY(buffer.buf+pos, buf, len);
}

void OS::Core::MemStreamWriter::writeByte(int32_t value)
{
	OS_ASSERT(value >= 0 && value <= 0xff);
	if(pos < buffer.count){
		OS_ASSERT(pos >= 0);
...
void OS::Core::MemStreamWriter::writeByteAtPos(int32_t value, int32_t pos)
{
	OS_ASSERT(value >= 0 && value <= 0xff);
...
int32_t OS::Core::FileStreamWriter::getPos() const
{
	return allocator->seekFile(f, 0, SEEK_CUR);
}

void OS::Core::FileStreamWriter::setPos(int32_t new_pos)
{
	OS_ASSERT(new_pos >= 0 && new_pos <= getSize());
	allocator->seekFile(f, new_pos, SEEK_SET);
}

int32_t OS::Core::FileStreamWriter::getSize() const
{
	if(f){
		int32_t save_pos = getPos();
...
void OS::Core::FileStreamWriter::writeBytes(const void * buf, int32_t len)
{
	allocator->writeFile(buf, len, f);
}

void OS::Core::FileStreamWriter::writeBytesAtPos(const void * buf, int32_t len, int32_t pos)
{
	int32_t save_pos = getPos();
...
OS_BYTE OS::Core::StreamReader::readByteAtPos(int32_t pos)
{
	OS_BYTE le_value;
...
int32_t OS::Core::StreamReader::readUVariable()
{
...
OS_U16 OS::Core::StreamReader::readU16AtPos(int32_t pos)
{
	OS_U16 le_value;
	readBytesAtPos(&le_value, sizeof(le_value), pos);
	return fromLittleEndianByteOrder(le_value);
...
OS_INT8 OS::Core::StreamReader::readInt8AtPos(int32_t pos)
{
	OS_INT8 le_value;
	readBytesAtPos(&le_value, sizeof(le_value), pos);
	return fromLittleEndianByteOrder(le_value);
...
OS_INT16 OS::Core::StreamReader::readInt16AtPos(int32_t pos)
{
	OS_INT16 le_value;
	readBytesAtPos(&le_value, sizeof(le_value), pos);
	return fromLittleEndianByteOrder(le_value);
...
OS_INT32 OS::Core::StreamReader::readInt32AtPos(int32_t pos)
{
	OS_INT32 le_value;
	readBytesAtPos(&le_value, sizeof(le_value), pos);
	return fromLittleEndianByteOrder(le_value);
...
OS_INT64 OS::Core::StreamReader::readInt64AtPos(int32_t pos)
{
	OS_INT64 le_value;
	readBytesAtPos(&le_value, sizeof(le_value), pos);
	return fromLittleEndianByteOrder(le_value);
...
float OS::Core::StreamReader::readFloatAtPos(int32_t pos)
{
	float le_value;
	readBytesAtPos(&le_value, sizeof(le_value), pos);
	return fromLittleEndianByteOrder(le_value);
...
double OS::Core::StreamReader::readDoubleAtPos(int32_t pos)
{
	double le_value;
	readBytesAtPos(&le_value, sizeof(le_value), pos);
	return fromLittleEndianByteOrder(le_value);
...
long double OS::Core::StreamReader::readLongDoubleAtPos(int32_t pos)
{
	long double le_value;
	readBytesAtPos(&le_value, sizeof(le_value), pos);
	return fromLittleEndianByteOrder(le_value);
...
OS::Core::MemStreamReader::MemStreamReader(OS * allocator, int32_t buf_size): StreamReader(allocator)
{
	cur = buffer = (OS_BYTE*)allocator->malloc(buf_size OS_DBG_FILEPOS);
	size = buf_size;
}

OS::Core::MemStreamReader::MemStreamReader(OS * allocator, OS_BYTE * buf, int32_t buf_size): StreamReader(allocator)
{
	cur = buffer = buf;
	size = buf_size;
}
...
int32_t OS::Core::MemStreamReader::getPos() const
{
	return (int32_t)(cur - buffer);
}

void OS::Core::MemStreamReader::setPos(int32_t new_pos)
{
	OS_ASSERT(new_pos >= 0 && new_pos <= size);
	cur = buffer + new_pos;
}

int32_t OS::Core::MemStreamReader::getSize() const
{
	return size;
}

void OS::Core::MemStreamReader::movePos(int32_t len)
{
	OS_ASSERT(getPos()+len >= 0 && getPos()+len <= size);
	cur += len;
}

bool OS::Core::MemStreamReader::checkBytes(const void * src, int32_t len)
{
...
void * OS::Core::MemStreamReader::readBytes(void * dst, int32_t len)
{
	OS_ASSERT(getPos() >= 0 && getPos()+len <= size);
...
void * OS::Core::MemStreamReader::readBytesAtPos(void * dst, int32_t len, int32_t pos)
{
...
{
	OS_ASSERT(getPos() >= 0 && getPos()+(int32_t)sizeof(OS_BYTE) <= size);
	return *cur++;
}

OS_BYTE OS::Core::MemStreamReader::readByteAtPos(int32_t pos)
{
	OS_ASSERT(pos >= 0 && pos+(int32_t)sizeof(OS_BYTE) <= size);
	return buffer[pos];
}
...
{
	OS_ASSERT(getPos() >= 0 && getPos()+(int32_t)sizeof(OS_INT16) <= size);
	OS_BYTE * buf = cur;
	cur += sizeof(OS_INT16);
...
{
	OS_ASSERT(getPos() >= 0 && getPos()+(int32_t)sizeof(OS_INT32) <= size);
	OS_BYTE * buf = cur;
	cur += sizeof(OS_INT32);
...
int32_t OS::Core::FileStreamReader::getPos() const
{
	return allocator->seekFile(f, 0, SEEK_CUR);
}

void OS::Core::FileStreamReader::setPos(int32_t new_pos)
{
	OS_ASSERT(new_pos >= 0 && new_pos <= getSize());
	allocator->seekFile(f, new_pos, SEEK_SET);
}

int32_t OS::Core::FileStreamReader::getSize() const
{
	if(f){
		int32_t save_pos = getPos();
		allocator->seekFile(f, 0, SEEK_END);
...
void OS::Core::FileStreamReader::movePos(int32_t len)
{
	allocator->seekFile(f, len, SEEK_CUR);
}

bool OS::Core::FileStreamReader::checkBytes(const void * src, int32_t len)
{
	void * buf = alloca(len);
	readBytes(buf, len);
...
void * OS::Core::FileStreamReader::readBytes(void * buf, int32_t len)
{
...
void * OS::Core::FileStreamReader::readBytesAtPos(void * buf, int32_t len, int32_t pos)
{
	int32_t save_pos = getPos();
	allocator->seekFile(f, pos, SEEK_SET);
	readBytes(buf, len);
...
bool OS::Core::GCStringValue::isEqual(int32_t hash, const void * b, int32_t size) const
{
...
bool OS::Core::GCStringValue::isEqual(int32_t hash, const void * buf1, int32_t size1, const void * buf2, int32_t size2) const
{
...
bool OS::Core::isEqual(const Value& index, int32_t hash, const void * b, int32_t size)
{
...
bool OS::Core::isEqual(const Value& index, int32_t hash, const void * buf1, int32_t size1, const void * buf2, int32_t size2) 
{
	if(OS_VALUE_TYPE(index) == OS_VALUE_TYPE_STRING){
		int32_t src_size = OS_VALUE_VARIANT(index).string->data_size;
...
template <class T> int32_t getNumberHash(T val)
{
	return (int32_t)val;
}
template <> int32_t getNumberHash<double>(double val)
{
	float t = (float)val;
	return *(int32_t*)&t;
}
template <> int32_t getNumberHash<float>(float t)
{
	return *(int32_t*)&t;
}
template <> int32_t getNumberHash<int32_t>(int32_t t)
{
	return t;
}
...
int32_t OS::Core::getValueHash(const Value& index, int32_t type)
{
	...
	case OS_VALUE_TYPE_NUMBER:
		{
			...
			int32_t i; OS_NUMBER_TO_INT(i, OS_VALUE_NUMBER(index));
			return i;
...
OS::Core::Property * OS::Core::addTableProperty(Table * table, const Value& index, const Value& value)
{
	...
	if((table->count>>HASH_GROW_SHIFT) >= table->head_mask){
		int32_t new_size = table->heads ? (table->head_mask+1) * 2 : 4;
		int32_t alloc_size = sizeof(Property*)*new_size;
		...
		for(Property * cur = table->first; cur; cur = cur->next){
			int32_t type = OS_VALUE_TYPE(cur->index);
#if 000
			int32_t hash;
			OS_CALC_VALUE_HASH(cur->index, type);
			int32_t slot = hash & table->head_mask;
#else
			int32_t slot = getValueHash(cur->index, type) & table->head_mask;
#endif
	...
	int32_t type = OS_VALUE_TYPE(prop->index);
#if 000
	int32_t hash;
	OS_CALC_VALUE_HASH(prop->index, type);
	int32_t slot = hash & table->head_mask;
#else
	int32_t slot = getValueHash(prop->index, type) & table->head_mask;
#endif
...
#ifdef OS_DEBUG
int32_t OS::Core::checkSavedType(int32_t type, const Value& value)
{
...
{
	OS_ASSERT(table);
	int32_t type = OS_VALUE_TYPE(index);
#if 000
	int32_t hash;
	OS_CALC_VALUE_HASH(index, type);
	int32_t slot = hash & table->head_mask;
#else
	int32_t slot = getValueHash(index, type) & table->head_mask;
#endif
	Property * cur = table->heads[slot], * chain_prev = NULL;
	for(; cur; chain_prev = cur, cur = cur->hash_next){
...
{
	int32_t index_type = OS_VALUE_TYPE(index);
	if(table_value->type == OS_VALUE_TYPE_ARRAY && index_type == OS_VALUE_TYPE_NUMBER){
		...
		int32_t i; OS_NUMBER_TO_INT(i, OS_VALUE_NUMBER(index)); // = (int32_t)valueToInt(index);
	...
	if(del_enabled /*&& !hasSpecialPrefix(index)*/){
		...
		if(OS_VALUE_TYPE(index) == OS_VALUE_TYPE_STRING){
			...
			int32_t size1 = strings->__delAt.getDataSize();
			const void * buf2 = OS_VALUE_VARIANT(index).string->toChar();
			int32_t size2 = OS_VALUE_VARIANT(index).string->getDataSize();
...
void OS::Core::sortTable(Table * table, int32_t(*comp)(OS*, const void*, const void*, void*), void * user_param, bool reorder_keys)
{
	if(table->count > 1){
...
void OS::Core::sortArray(GCArrayValue * arr, int32_t(*comp)(OS*, const void*, const void*, void*), void * user_param)
{
	allocator->qsort(arr->values.buf, arr->values.count, sizeof(Value), comp, user_param);
}

static int32_t compareResult(OS_NUMBER num)
{
	if(num < 0) return -1;
	if(num > 0) return 1;
	return 0;
...
int32_t OS::Core::comparePropValues(OS * os, const void * a, const void * b, void*)
{
...
int32_t OS::Core::comparePropValuesReverse(OS * os, const void * a, const void * b, void * user_param)
{
	return comparePropValues(os, b, a, user_param);
}

int32_t OS::Core::compareObjectProperties(OS * os, const void * a, const void * b, void * user_param)
{
	Property * props[] = {*(Property**)a, *(Property**)b};
...
int32_t OS::Core::compareObjectPropertiesReverse(OS * os, const void * a, const void * b, void * user_param)
{
	return compareObjectProperties(os, b, a, user_param);
}

int32_t OS::Core::compareUserPropValues(OS * os, const void * a, const void * b, void*)
{
	Property * props[] = {*(Property**)a, *(Property**)b};
	os->pushStackValue(-1);
	os->core->pushValue(props[0]->value);
...
int32_t OS::Core::compareUserPropValuesReverse(OS * os, const void * a, const void * b, void * user_param)
{
	return compareUserPropValues(os, b, a, user_param);
}

int32_t OS::Core::comparePropKeys(OS * os, const void * a, const void * b, void*)
{
	Property * props[] = {*(Property**)a, *(Property**)b};
...
int32_t OS::Core::comparePropKeysReverse(OS * os, const void * a, const void * b, void * user_param)
{
	return comparePropKeys(os, b, a, user_param);
}

int32_t OS::Core::compareUserPropKeys(OS * os, const void * a, const void * b, void*)
{
	Property * props[] = {*(Property**)a, *(Property**)b};
	os->pushStackValue(-1);
	os->core->pushValue(props[0]->index);
...
int32_t OS::Core::compareUserPropKeysReverse(OS * os, const void * a, const void * b, void * user_param)
{
	return compareUserPropKeys(os, b, a, user_param);
}

int32_t OS::Core::compareArrayValues(OS * os, const void * a, const void * b, void*)
{
...
int32_t OS::Core::compareArrayValuesReverse(OS * os, const void * a, const void * b, void * user_param)
{
	return compareArrayValues(os, b, a, user_param);
}

int32_t OS::Core::compareUserArrayValues(OS * os, const void * a, const void * b, void*)
{
	Value * values[] = {(Value*)a, (Value*)b};
	os->pushStackValue(-1);
...
int32_t OS::Core::compareUserArrayValuesReverse(OS * os, const void * a, const void * b, void * user_param)
{
	return compareUserArrayValues(os, b, a, user_param);
}

int32_t OS::Core::compareUserReverse(OS * os, const void * a, const void * b, void * user_param)
{
	int32_t (*comp)(OS*, const void*, const void*, void*) = (int32_t(*)(OS*, const void*, const void*, void*))user_param;
	return comp(os, b, a, NULL);
}

OS::Core::Property * OS::Core::Table::get(const Value& index, int32_t type)
{
	if(heads){
		OS_ASSERT(OS_VALUE_TYPE(index) == type);
#if 000
...
OS::Core::Locals * OS::Core::Locals::getParent(int32_t i)
{
...
OS::Core::GCStringValue::GCStringValue(int32_t p_data_size)
{
...
OS::Core::GCStringValue * OS::Core::GCStringValue::allocAndPush(OS * allocator, int32_t p_hash, const void * buf, int32_t data_size OS_DBG_FILEPOS_DECL)
{
	OS_ASSERT(data_size >= 0);
	if(data_size < 0) data_size = 0;
	int32_t alloc_size = data_size + sizeof(GCStringValue) + sizeof(wchar_t) + sizeof(wchar_t)/2;
...
OS::Core::GCStringValue * OS::Core::GCStringValue::allocAndPush(OS * allocator, int32_t p_hash, const void * buf1, int32_t len1, const void * buf2, int32_t len2 OS_DBG_FILEPOS_DECL)
{
	OS_ASSERT(len1 >= 0 && len2 >= 0);
	if(len1 < 0) len1 = 0;
	if(len2 < 0) len2 = 0;
	int32_t alloc_size = len1 + len2 + sizeof(GCStringValue) + sizeof(wchar_t) + sizeof(wchar_t)/2;
...
bool OS::Core::GCStringValue::isNumberRadix(int32_t radix, OS_NUMBER* p_val) const
{
	const OS_CHAR * str = toChar();
	const OS_CHAR * end = str + getLen();
	OS_FLOAT val;
...
OS_NUMBER OS::Core::GCStringValue::toNumberRadix(int32_t radix) const
{
	const OS_CHAR * str = toChar();
	const OS_CHAR * end = str + getLen();
	OS_FLOAT val;
...
int32_t OS::Core::GCStringValue::cmp(GCStringValue * string) const
{
	if(this == string){
		return 0;
	}
...
int32_t OS::Core::GCStringValue::cmp(const OS_CHAR * str) const
{
	return cmp(str, (int32_t)OS_STRLEN(str));
}

int32_t OS::Core::GCStringValue::cmp(const OS_CHAR * str, int32_t len) const
{
...
OS_INT OS::Core::valueToIntRadix(const Value& val, int32_t radix, bool valueof_enabled)
{
...
OS_NUMBER OS::Core::valueToNumberRadix(const Value& val, int32_t radix, bool valueof_enabled)
{
	switch(OS_VALUE_TYPE(val)){
	case OS_VALUE_TYPE_NULL:
		return 0; // nan_float;
...
OS::Core::String OS::Core::Compiler::Expression::toString()
{
	...
	case EXP_TYPE_CONST_NUMBER:
		...
		return String(getAllocator(), token->getInt());
...
void OS::Core::deleteStringRefs()
{
	...
	for(int32_t i = 0; i <= string_refs.head_mask; i++){
...
void OS::Core::registerUserptrRef(GCUserdataValue * user_pointer_ref)
{
	...
	if((userptr_refs.count>>HASH_GROW_SHIFT) >= userptr_refs.head_mask){
		int32_t new_size = userptr_refs.heads ? (userptr_refs.head_mask+1) * 2 : 32;
		int32_t alloc_size = sizeof(GCUserdataValue*) * new_size;
		GCUserdataValue ** new_heads = (GCUserdataValue**)malloc(alloc_size OS_DBG_FILEPOS);
		OS_ASSERT(new_heads);
		OS_MEMSET(new_heads, 0, alloc_size);

		GCUserdataValue ** old_heads = userptr_refs.heads;
		int32_t old_mask = userptr_refs.head_mask;
		...
		if(old_heads){
			for(int32_t i = 0; i <= old_mask; i++){
				for(GCUserdataValue * user_pointer_ref = old_heads[i], * next; user_pointer_ref; user_pointer_ref = next){
					...
					int32_t slot = OS_PTR_HASH(user_pointer_ref->ptr) & userptr_refs.head_mask;
...
{
	int32_t slot = OS_PTR_HASH(userptr_ref->ptr) & userptr_refs.head_mask;
...
void OS::Core::unregisterUserptrRef(void * ptr, int32_t value_id)
{
	if(userptr_refs.count > 0){
		OS_ASSERT(userptr_refs.heads && userptr_refs.head_mask);
		int32_t hash = OS_PTR_HASH(ptr);
		int32_t slot = hash & userptr_refs.head_mask;
...
void OS::Core::deleteUserptrRefs()
{
	...
	for(int32_t i = 0; i <= userptr_refs.head_mask; i++){
...
void OS::Core::registerCFuncRef(GCCFunctionValue * cfunc_ref)
{
	...
	if((cfunc_refs.count>>HASH_GROW_SHIFT) >= cfunc_refs.head_mask){
		int32_t new_size = cfunc_refs.heads ? (cfunc_refs.head_mask+1) * 2 : 32;
		int32_t alloc_size = sizeof(GCCFunctionValue*) * new_size;
		GCCFunctionValue ** new_heads = (GCCFunctionValue**)malloc(alloc_size OS_DBG_FILEPOS);
		OS_ASSERT(new_heads);
		OS_MEMSET(new_heads, 0, alloc_size);

		GCCFunctionValue ** old_heads = cfunc_refs.heads;
		int32_t old_mask = cfunc_refs.head_mask;
		...
		if(old_heads){
			for(int32_t i = 0; i <= old_mask; i++){
				for(GCCFunctionValue * cfunc_ref = old_heads[i], * next; cfunc_ref; cfunc_ref = next){
					...
					int32_t slot = cfunc_ref->cfunc_hash & cfunc_refs.head_mask;
...
void OS::Core::unregisterCFuncRef(OS_CFunction func, void * user_param, int32_t value_id)
{
	if(cfunc_refs.count > 0){
		OS_ASSERT(cfunc_refs.heads && cfunc_refs.head_mask);
		int32_t hash = Utils::keyToHash(&func, sizeof(func), user_param ? &user_param : NULL, user_param ? sizeof(user_param) : 0);
		int32_t slot = hash & cfunc_refs.head_mask;
...
void OS::Core::deleteCFuncRefs()
{
	...
	for(int32_t i = 0; i <= cfunc_refs.head_mask; i++){
...
void OS::Core::registerValueAndPush(GCValue * value)
{
	...
	if((values.count>>HASH_GROW_SHIFT) >= values.head_mask){
		int32_t new_size = values.heads ? (values.head_mask+1) * 2 : 32;
		int32_t alloc_size = sizeof(GCValue*) * new_size;
		GCValue ** new_heads = (GCValue**)malloc(alloc_size OS_DBG_FILEPOS); // new Value*[new_size];
		OS_ASSERT(new_heads);
		OS_MEMSET(new_heads, 0, alloc_size);

		GCValue ** old_heads = values.heads;
		int32_t old_mask = values.head_mask;
		...
		if(old_heads){
			for(int32_t i = 0; i <= old_mask; i++){
				for(GCValue * value = old_heads[i], * next; value; value = next){
					...
					int32_t slot = value->value_id & values.head_mask;
...
OS::Core::GCValue * OS::Core::unregisterValue(int32_t value_id)
{
	OS_ASSERT(!gc_candidate_values.get(value_id));
	int32_t slot = value_id & values.head_mask;
...
void OS::Core::deleteValues(bool del_ref_counted_also)
{
	if(values.heads && values.count > 0){
		while(true){
			for(int32_t i = 0; i <= values.head_mask; i++){
#if 0
...
OS::Core::GCValue * OS::Core::Values::get(int32_t value_id)
{
	int32_t slot = value_id & head_mask;
...
void OS::Core::registerFreeCandidateValue(GCValue * value)
{
	...
	if((gc_candidate_values.count>>HASH_GROW_SHIFT) >= gc_candidate_values.head_mask){
		int32_t new_size = gc_candidate_values.heads ? (gc_candidate_values.head_mask+1) * 2 : 32;
		int32_t alloc_size = sizeof(GCValue*) * new_size;
		GCValue ** new_heads = (GCValue**)malloc(alloc_size OS_DBG_FILEPOS); // new Value*[new_size];
		OS_ASSERT(new_heads);
		OS_MEMSET(new_heads, 0, alloc_size);
		...
		int32_t old_mask = gc_candidate_values.head_mask;
		...
		if(old_heads){
			for(int32_t i = 0; i <= old_mask; i++){
				for(GCValue * value = old_heads[i], * next; value; value = next){
					...
					int32_t slot = value->value_id & gc_candidate_values.head_mask;
		...
		int32_t new_size = (gc_candidate_values.head_mask+1) / 2;
		int32_t alloc_size = sizeof(GCValue*) * new_size;
		GCValue ** new_heads = (GCValue**)malloc(alloc_size OS_DBG_FILEPOS); // new Value*[new_size];
		OS_ASSERT(new_heads);
		OS_MEMSET(new_heads, 0, alloc_size);
...
void OS::Core::deleteFreeCandidateValues()
{
	if(gc_candidate_values.heads && gc_candidate_values.count > 0){
		while(true){
			for(int32_t i = 0; i <= gc_candidate_values.head_mask; i++){
...
OS::Core::GCValue * OS::Core::FreeCandidateValues::get(int32_t value_id)
{
	if(!heads){
		return NULL;
	}
...
void OS::Core::gcFreeCandidateValues(bool full)
{
	...
	{
		OS * allocator;
		int32_t gc_step_type;
		
		/*
		GCValue * destroy_list;
		int32_t destroy_list_size;
		...
		*/
		...
		void destroyValues(bool fix)
		{
			if(destroy_list_size > 0){		
				...
				for(int32_t i = collected_values.count-1; i >= 0; i--){
		...
		*/
		...
		void mark(StackFunction * stack_func)
		{
			...
			for(int32_t i = 0; i < stack_func->sub_funcs.count; i++){
			...
			case OS_VALUE_TYPE_ARRAY:
				{
					...
					for(int32_t i = 0; i < arr->values.count; i++){
						mark(arr->values[i]);
					}
					break;
				}
			...
			case OS_VALUE_TYPE_FUNCTION:
				{
					...
					for(int32_t i = 0; i < func_value->prog->num_strings + func_value->prog->num_numbers + CONST_STD_VALUES; i++){
			...
			case OS_VALUE_TYPE_CFUNCTION:
				{
					...
					for(int32_t i = 0; i < func_value->num_closure_values; i++){
		...
		{
			int32_t head_mask = allocator->core->values.head_mask;
			for(int32_t i = 0; i <= head_mask; i++){
	...
	// int32_t valid_mark = num_created_values - 64; // wait some time for new allocated values
	int32_t head_mask = gc_candidate_values.head_mask;
	...
	int32_t used_bytes = allocator->getUsedBytes();
	if(full || used_bytes >= gc_next_when_used_bytes){		
		...
		int32_t save_next_id = values.next_id;
...
void OS::Core::dumpValues(Buffer& out)
{
	struct Lib 
	{
		...
		void appendDepthSpaces(int32_t depth)
		{
		...
		void dump(StackFunction * stack_func, int32_t depth)
		{
			OS_ASSERT(stack_func->func);
		...
		void dump(const Value& cur, int32_t depth)
		{
		...
		void dump(GCValue * value, int32_t depth)
		{
...
{
	buf += OS_TEXT("\"");
	int32_t len = string.getLen();
	const OS_CHAR * str = string.toChar();
	for(int32_t i = 0; i < len; i++, str++){
		switch(*str){
		...
		if(*str < OS_TEXT(' ')){
			buf += OS_TEXT("0x");
			buf.append(UPPER_DIGITS[((int32_t)*str >> 4) & 0xF]);
			buf.append(UPPER_DIGITS[((int32_t)*str >> 0) & 0xF]);
			continue;
		}
		buf.append(*str);
	}
...
int32_t OS::getFileSize(const OS_CHAR * filename)
{
	FileHandle * f = openFile(filename, "rb");
	if(f){
...
int32_t OS::getFileSize(FileHandle * f)
{
	if(f){
		int32_t pos = seekFile(f, 0, SEEK_CUR);
		int32_t size = seekFile(f, 0, SEEK_END);
...
int32_t OS::readFile(void * buf, int32_t size, FileHandle * f)
{
	if(f){
		return (int32_t)fread(buf, size, 1, (FILE*)f) * size;
	}
	return 0;
}

int32_t OS::writeFile(const void * buf, int32_t size, FileHandle * f)
{
	if(f){
		return (int32_t)fwrite(buf, size, 1, (FILE*)f) * size;
	}
	return 0;
}

int32_t OS::seekFile(FileHandle * f, int32_t offset, int32_t whence)
{
	if(f){
...
void OS::echo(const void * buf, int32_t size)
{
	OS_OUTPUT(buf, size);
}
...
{
	echo((void*)str, (int32_t)OS_STRLEN(str) * sizeof(OS_CHAR));
}
...
#ifdef OS_DEBUG
	int32_t mark = 0;
	native_stack_start_mark = (int32_t)&mark;
...
void OS::checkNativeStackUsage(const OS_CHAR * func_name)
{
	...
	int32_t mark = 0;
	int32_t cur_native_stack_usage = (int32_t)&mark - native_stack_start_mark;
...
void * OS::malloc(int32_t size OS_DBG_FILEPOS_DECL)
{
...
void * OS::Core::malloc(int32_t size OS_DBG_FILEPOS_DECL)
{
	return allocator->malloc(size OS_DBG_FILEPOS_PARAM);
}
...
int32_t OS::getTerminatedCode()
{
	return core->terminated_code;
}

void OS::setTerminated(bool terminated, int32_t code)
{
...
OS::Core::~Core()
{
	...
	for(int32_t i = 0; i < PROTOTYPE_COUNT; i++){
...
int32_t OS::Core::compareGCValues(const void * a, const void * b)
{
...
void OS::Core::shutdown()
{
	...
	*/
	...
	gc_fix_in_progress = true;
	GCValue * destroy_list = NULL;
	for(int32_t j = 0; j < 10 && values.count > 0; j++){
		for(i = 0; i <= values.head_mask; i++){
			GCValue * value = values.heads[i], * prev = NULL, * next;
			for(; value; value = next){
				next = value->hash_next;
	...
	GCValue * destroy_list = NULL;
	int32_t destroy_list_size = 0;
	const int32_t MAX_FIX_TIMES = 10;
	for(int32_t j = 0; j < MAX_FIX_TIMES && values.count; j++){
		for(i = 0; i <= values.head_mask; i++){
...
OS::String OS::changeFilenameExt(const String& filename, const OS_CHAR * ext)
{
	...
	for(int32_t i = len-1; i >= 0; i--){
		if(filename[i] == OS_TEXT('.')){
			...
			return String(this, filename, i, ext, (int32_t)OS_STRLEN(ext));
		}
		if(OS_IS_SLASH(filename[i])){
			break;
		}
	}
	return String(this, filename, len, ext, (int32_t)OS_STRLEN(ext));
}
...
{
	return getFilenameExt(filename, (int32_t)OS_STRLEN(filename));
}

OS::String OS::getFilenameExt(const OS_CHAR * filename, int32_t len)
{
	for(int32_t i = len-1; i >= 0; i--){
		if(filename[i] == OS_TEXT('.')){
...
{
	return getFilename(filename, (int32_t)OS_STRLEN(filename));
}

OS::String OS::getFilename(const OS_CHAR * filename, int32_t len)
{
	for(int32_t i = len-1; i >= 0; i--){
		if(OS_IS_SLASH(filename[i])){
...
{
	return getFilenamePath(filename, (int32_t)OS_STRLEN(filename));
}

OS::String OS::getFilenamePath(const OS_CHAR * filename, int32_t len)
{
	for(int32_t i = len-1; i >= 0; i--){
		if(OS_IS_SLASH(filename[i])){
...
OS::String OS::resolvePath(const String& filename)
{
	...
	if(core->call_stack_funcs.count > 0){
		for(int32_t i = core->call_stack_funcs.count-1; i >= 0; i--){
...
OS::Core::DebugInfo OS::Core::getDebugInfo()
{
	...
	for(int32_t i = call_stack_funcs.count-1; i >= 0 && !debug_info; i--){
		Core::StackFunction * stack_func = call_stack_funcs.buf + i;
		prog = stack_func->func->prog;
		if(!prog->filename.isEmpty()){
			int32_t opcode_pos = (int32_t)(stack_func->opcodes - prog->opcodes.buf);
...
void OS::Core::retainValues(const Value * list, int32_t count)
{
	for(int32_t i = 0; i < count; i++){
		retainValue(list[i]);
	}
...
void OS::Core::releaseValues(const Value * list, int32_t count)
{
	for(int32_t i = 0; i < count; i++){
		releaseValue(list[i]);
	}
...
void OS::Core::releaseValuesAndClear(Value * list, int32_t count)
{
	for(int32_t i = 0; i < count; i++){
...
#define OS_SETTER_VALUE_PTR(_table_value, _index, _index_type, _value, _setter_enabled) \
	do { \
		...
		int32_t local7_index_type = (_index_type); \
		...
		if(local7_table_value->type == OS_VALUE_TYPE_ARRAY && local7_index_type == OS_VALUE_TYPE_NUMBER){ \
			...
			int32_t i; OS_NUMBER_TO_INT(i, OS_VALUE_NUMBER(local7_index)); \
		...
		if(local7_setter_enabled /*&& !hasSpecialPrefix(local7_index_copy)*/){ \
			...
			if(local7_index_type == OS_VALUE_TYPE_STRING){ \
				...
				int32_t size1 = strings->__setAt.getDataSize(); \
				const void * buf2 = OS_VALUE_VARIANT(local7_index_copy).string->toChar(); \
				int32_t size2 = OS_VALUE_VARIANT(local7_index_copy).string->getDataSize(); \
...
#define OS_SETTER_VALUE(_table_value, _index, _index_type, _value, _setter_enabled) \
	do { \
		...
		int32_t local8_index_type = (_index_type); \
		OS_ASSERT(OS_VALUE_TYPE(local8_index) == (local8_index_type)); \
		const Value& local8_value = (_value); \
		const int32_t type = OS_VALUE_TYPE(local8_table_value); \
		if(type == OS_VALUE_TYPE_ARRAY && local8_index_type == OS_VALUE_TYPE_NUMBER){ \
			...
			int32_t i; OS_NUMBER_TO_INT(i, OS_VALUE_NUMBER(local8_index)); \
			if(i >= 0 || (i += arr->values.count) >= 0){ \
				if(i == arr->values.count){ \
...
void OS::Core::setPrototypeValue(const Value& val, const Value& proto, int32_t userdata_crc)
{
	switch(OS_VALUE_TYPE(val)){
	case OS_VALUE_TYPE_USERDATA:
	case OS_VALUE_TYPE_USERPTR:
...
{
	return pushStringValue(str, (int32_t)OS_STRLEN(str));
}

OS::Core::GCStringValue * OS::Core::pushStringValue(const OS_CHAR * str, int32_t len)
{
	return pushStringValue((void*)str, len * sizeof(OS_CHAR));
}

OS::Core::GCStringValue * OS::Core::pushStringValue(const OS_CHAR * str, int32_t len, const OS_CHAR * str2, int32_t len2)
{
	return pushStringValue((void*)str, len * sizeof(OS_CHAR), str2, len2 * sizeof(OS_CHAR));
}

OS::Core::GCStringValue * OS::Core::pushStringValue(const OS_CHAR * str, int32_t len, bool trim_left, bool trim_right)
{
	if(trim_left){
...
OS::Core::GCStringValue * OS::Core::pushStringValue(const void * buf, int32_t size)
{
	return pushStringValue(buf, size, NULL, 0);
}

OS::Core::GCStringValue * OS::Core::pushStringValue(const void * buf1, int32_t size1, const void * buf2, int32_t size2)
{
	...
	if(string_refs.count > 0){
		...
		int32_t slot = hash & string_refs.head_mask;
...
OS::Core::GCStringValue * OS::Core::pushStringValue(const void * buf1, int32_t size1, const void * buf2, int32_t size2, const void * buf3, int32_t size3)
{
	if(size1 <= 0){
...
OS::Core::GCStringValue * OS::Core::pushStringValue(OS_FLOAT val, int32_t precision)
{
	OS_CHAR str[128];
...
OS::Core::GCStringValue * OS::Core::pushStringValue(int32_t temp_buf_len, const OS_CHAR * fmt, ...)
{
	va_list va;
	va_start(va, fmt);
	OS_VaListDtor va_dtor(&va);
...
OS::Core::GCStringValue * OS::Core::pushStringValueVa(int32_t temp_buf_len, const OS_CHAR * fmt, va_list va)
{
...
OS::Core::GCCFunctionValue * OS::Core::pushCFunctionValue(OS_CFunction func, int32_t num_closure_values, void * user_param)
{
...
OS::Core::GCUserdataValue * OS::Core::pushUserdataValue(int32_t crc, int32_t data_size, OS_UserdataDtor dtor, void * user_param)
{
	gcFreeCandidateValues();
...
int32_t OS::findUserPointerValueId(void * data)
{
...
OS::Core::GCUserdataValue * OS::Core::pushUserPointerValue(int32_t crc, void * ptr, OS_UserdataDtor dtor, void * user_param)
{
	int32_t hash = OS_PTR_HASH(ptr);
	if(userptr_refs.count > 0){
		OS_ASSERT(userptr_refs.heads && userptr_refs.head_mask > 0);
...
OS::Core::GCArrayValue * OS::Core::pushArrayValue(int32_t initial_capacity)
{
	gcFreeCandidateValues();
...
void OS::Core::pushStackValue(int32_t offs)
{
	pushValue(getStackValue(offs));
}

void OS::Core::copyValue(int32_t raw_from, int32_t raw_to)
{
	...
	case OS_VALUE_TYPE_ARRAY:
		{
			...
			for(int32_t i = 0; i < src_arr->values.count; i++){
...
bool OS::Core::pushOpResultValue(OpcodeType opcode, const Value& left_value, const Value& right_value)
{
	...
	OS_NUMBER right;
	int32_t is_gc_left_value = 0;
	int32_t left_type = OS_VALUE_TYPE(left_value);
	int32_t right_type = OS_VALUE_TYPE(right_value);
	...
	case OS_VALUE_TYPE_BOOL:
		...
		case OS_VALUE_TYPE_BOOL:
			...
			case OP_COMPARE:
				if(!exist){
					int32_t left_null = left_type != OS_VALUE_TYPE_NULL;
					int32_t right_null = right_type != OS_VALUE_TYPE_NULL;
...
int32_t OS::Core::getStackOffs(int32_t offs)
{
	return offs < 0 ? stack_values.count + offs : offs;
}

OS::Core::Value OS::Core::getStackValue(int32_t offs)
{
	StackValues& stack_values = this->stack_values;
...
void OS::Core::reserveStackValues(int32_t new_capacity)
{
...
void OS::Core::growStackValues(int32_t new_capacity)
{
	...
	for(int32_t i = 0; i < call_stack_funcs.count; i++){
...
void OS::Core::removeStackValues(int32_t offs, int32_t count)
{
	if(count <= 0){
		OS_ASSERT(count == 0);
		return;
	...
	StackValues& stack_values = this->stack_values;
	int32_t start = offs < 0 ? stack_values.count + offs : offs;
...
void OS::Core::removeStackValue(int32_t offs)
{
...
void OS::Core::pop(int32_t count)
{
	StackValues& stack_values = this->stack_values;
#if 0 // def OS_DEBUG
...
void OS::Core::moveStackValues(int32_t offs, int32_t count, int32_t new_offs)
{
	if(count <= 0){
		OS_ASSERT(count == 0);
		return;
	...
	int32_t new_end = new_offs + count;
...
void OS::Core::moveStackValue(int32_t offs, int32_t new_offs)
{
	StackValues& stack_values = this->stack_values;
	offs = offs < 0 ? stack_values.count + offs : offs;
	if(offs < 0 || offs >= stack_values.count){
...
void OS::Core::exchangeStackValues(int32_t offs)
{
	offs = offs < 0 ? stack_values.count + offs : offs;
...
void OS::Core::insertValue(Value val, int32_t offs)
{
	offs = offs < 0 ? stack_values.count + offs : offs;
	...
	int32_t count = stack_values.count - offs - 1;
	if(count > 0){
...
void OS::pushString(const OS_CHAR * val, int32_t len)
{
	core->pushStringValue(val, len);
}

void OS::pushString(const void * val, int32_t size)
{
...
void OS::pushCFunction(OS_CFunction func, int32_t closure_values, void * user_param)
{
	core->pushCFunctionValue(func, closure_values, user_param);
}

void * OS::pushUserdata(int32_t crc, int32_t data_size, OS_UserdataDtor dtor, void * user_param)
{
	Core::GCUserdataValue * userdata = core->pushUserdataValue(crc, data_size, dtor, user_param);
	return userdata ? userdata->ptr : NULL;
}

void * OS::pushUserdata(int32_t data_size, OS_UserdataDtor dtor, void * user_param)
{
	return pushUserdata(0, data_size, dtor, user_param);
}

void * OS::pushUserPointer(int32_t crc, void * data, OS_UserdataDtor dtor, void * user_param)
{
...
void OS::newArray(int32_t initial_capacity)
{
	core->pushArrayValue(initial_capacity);
}

void OS::pushBackTrace(int32_t skip_funcs, int32_t max_trace_funcs)
{
	core->pushBackTrace(skip_funcs, max_trace_funcs);
}

void OS::pushStackValue(int32_t offs)
{
...
void OS::pushValueById(int32_t id)
{
	core->pushValue(core->values.get(id));
}

void OS::retainValueById(int32_t id)
{
...
void OS::releaseValueById(int32_t id)
{
	Core::GCValue * value = core->values.get(id);
	if(value){
		Core::Value val;
		if(!core->getPropertyValue(val, core->retain_pool, value, false)){
			...
			int32_t ref_count = (int32_t)core->valueToNumber(val) - 1;
			if(ref_count > 0){
...
void OS::clone(int32_t offs)
{
	core->pushCloneValue(core->getStackValue(offs));
}

int32_t OS::getStackSize()
{
	return core->stack_values.count;
}

int32_t OS::getAbsoluteOffs(int32_t offs)
{
	return core->getStackOffs(offs);
}

void OS::remove(int32_t start_offs, int32_t count)
{
	core->removeStackValues(start_offs, count);
}

void OS::pop(int32_t count)
{
	core->pop(count);
}
...
void OS::move(int32_t start_offs, int32_t count, int32_t new_offs)
{
	core->moveStackValues(start_offs, count, new_offs);
}

void OS::move(int32_t offs, int32_t new_offs)
{
	core->moveStackValue(offs, new_offs);
}

void OS::exchange(int32_t offs)
{
	core->exchangeStackValues(offs);
}

bool OS::toBool(int32_t offs)
{
	return core->valueToBool(core->getStackValue(offs));
}

bool OS::toBool(int32_t offs, bool def)
{
	Core::Value value = core->getStackValue(offs);
	return value.isNull() ? def : core->valueToBool(value);
}

OS_NUMBER OS::toNumber(int32_t offs, bool valueof_enabled)
{
	return core->valueToNumber(core->getStackValue(offs), valueof_enabled);
}

OS_NUMBER OS::toNumber(int32_t offs, OS_NUMBER def, bool valueof_enabled)
{
	Core::Value value = core->getStackValue(offs);
	return value.isNull() ? def : core->valueToNumber(value, valueof_enabled);
}

float OS::toFloat(int32_t offs, bool valueof_enabled)
{
	return (float)toNumber(offs, valueof_enabled);
}

float OS::toFloat(int32_t offs, float def, bool valueof_enabled)
{
	return (float)toNumber(offs, (OS_NUMBER)def, valueof_enabled);
}

double OS::toDouble(int32_t offs, bool valueof_enabled)
{
	return (double)toNumber(offs, valueof_enabled);
}

double OS::toDouble(int32_t offs, double def, bool valueof_enabled)
{
	return (double)toNumber(offs, (OS_NUMBER)def, valueof_enabled);
}

int32_t OS::toInt(int32_t offs, bool valueof_enabled)
{
	return (int32_t)toNumber(offs, valueof_enabled);
}

int32_t OS::toIntRadix(int32_t offs, int32_t radix, bool valueof_enabled)
{
	return (int32_t)core->valueToNumberRadix(core->getStackValue(offs), radix, valueof_enabled);
}

int32_t OS::toInt(int32_t offs, int32_t def, bool valueof_enabled)
{
	return (int32_t)toNumber(offs, (OS_NUMBER)def, valueof_enabled);
}

int32_t OS::toIntRadix(int32_t offs, int32_t def, int32_t radix, bool valueof_enabled)
{
	Core::Value value = core->getStackValue(offs);
	return value.isNull() ? def : (int32_t)core->valueToNumberRadix(value, radix, valueof_enabled);
}

bool OS::isNumber(int32_t offs, OS_NUMBER * out)
{
	return core->isValueNumber(core->getStackValue(offs), out);
}

OS::String OS::toString(int32_t offs, bool valueof_enabled)
{
	return core->valueToStringOS(core->getStackValue(offs), valueof_enabled);
}

OS::String OS::toString(int32_t offs, const String& def, bool valueof_enabled)
{
	Core::Value value = core->getStackValue(offs);
	return value.isNull() ? def : core->valueToStringOS(value, valueof_enabled);
}

OS::String OS::toString(int32_t offs, const OS_CHAR * def, bool valueof_enabled)
{
	Core::Value value = core->getStackValue(offs);
	return value.isNull() ? String(this, def) : core->valueToStringOS(value, valueof_enabled);
}

bool OS::isString(int32_t offs, String * out)
{
...
int32_t OS::popInt(bool valueof_enabled)
{
	Pop pop(this); (void)pop;
	return toInt(-1, valueof_enabled);
}

int32_t OS::popIntRadix(int32_t radix, bool valueof_enabled)
{
	Pop pop(this); (void)pop;
	return toIntRadix(-1, radix, valueof_enabled);
}

int32_t OS::popInt(int32_t def, bool valueof_enabled)
{
	Pop pop(this); (void)pop;
	return toInt(-1, def, valueof_enabled);
}

int32_t OS::popIntRadix(int32_t def, int32_t radix, bool valueof_enabled)
{
	Pop pop(this); (void)pop;
...
OS_EValueType OS::getType(int32_t offs)
{
	Core::Value value = core->getStackValue(offs);
	return (OS_EValueType)OS_VALUE_TYPE(value);
}

OS_EValueType OS::getTypeById(int32_t id)
{
...
OS::String OS::getTypeStr(int32_t offs)
{
	core->pushTypeOf(core->getStackValue(offs));
	return popString();
}

OS::String OS::getTypeStrById(int32_t id)
{
	core->pushTypeOf(core->values.get(id));
	return popString();
}

bool OS::isType(OS_EValueType type, int32_t offs)
{
	return OS_VALUE_TYPE(core->getStackValue(offs)) == type;
}

bool OS::isNull(int32_t offs)
{
	return isType(OS_VALUE_TYPE_NULL, offs);
}

bool OS::isObject(int32_t offs)
{
...
bool OS::isUserdata(int32_t offs)
{
...
bool OS::isUserdata(int32_t crc, int32_t offs, int32_t prototype_crc)
{
	Core::Value val = core->getStackValue(offs);
	switch(OS_VALUE_TYPE(val)){
	case OS_VALUE_TYPE_USERDATA:
...
void * OS::toUserdata(int32_t crc, int32_t offs, int32_t prototype_crc)
{
	Core::Value val = core->getStackValue(offs);
	switch(OS_VALUE_TYPE(val)){
	case OS_VALUE_TYPE_USERDATA:
...
void OS::clearUserdata(int32_t crc, int32_t offs, int32_t prototype_crc)
{
	Core::Value val = core->getStackValue(offs);
	switch(OS_VALUE_TYPE(val)){
	case OS_VALUE_TYPE_USERDATA:
...
bool OS::isArray(int32_t offs)
{
	return isType(OS_VALUE_TYPE_ARRAY, offs);
}

bool OS::isFunction(int32_t offs)
{
...
bool OS::Core::isValueOfUserdata(GCValue * val, int32_t prototype_crc)
{
	for(int32_t value_crc;;){
		switch(val->type){
		case OS_VALUE_TYPE_USERDATA:
		case OS_VALUE_TYPE_USERPTR:
			OS_ASSERT(dynamic_cast<GCUserdataValue*>(val));
...
bool OS::is(int32_t value_offs, int32_t prototype_offs)
{
...
bool OS::Core::isValueInValue(const Value& _name, const Value& _obj)
{
	...
	if(self){
		if(self->type == OS_VALUE_TYPE_ARRAY){
			...
			int32_t count = arr->values.count;
			for(int32_t i = 0; i < count; i++){
...
bool OS::in(int32_t name_offs, int32_t obj_offs)
{
...
void OS::setProperty(int32_t offs, const OS_CHAR * name, bool setter_enabled)
{
	setProperty(offs, Core::String(this, name), setter_enabled);
}

void OS::setProperty(int32_t offs, const Core::String& name, bool setter_enabled)
{
...
void OS::addProperty(int32_t offs, bool setter_enabled)
{
	OS_INT i;
	Core::Value object = core->getStackValue(offs);
	switch(OS_VALUE_TYPE(object)){
...
{
	int32_t offs = getAbsoluteOffs(-2);
	bool index = false;
	...
	for(bool initialized = false; cur < end;){
		...
		if(begin_index == end_index){
			...
			pushString(begin_index, (int32_t)(end_index - begin_index));
			initialized = true;
		}
	}
	int32_t count = getAbsoluteOffs(-2) - offs;
...
void OS::setPrototype(int32_t userdata_crc)
{
	if(core->stack_values.count >= 2){
...
int32_t OS::getCFuncClosureCount(int32_t offs)
{
...
void OS::getCFuncClosure(int32_t offs, int32_t i)
{
	Core::Value func = core->getStackValue(offs);
	if(OS_VALUE_TYPE(func) == OS_VALUE_TYPE_CFUNCTION){
		OS_ASSERT(dynamic_cast<Core::GCCFunctionValue*>(OS_VALUE_VARIANT(func).cfunc));
...
void OS::getCFuncClosure(int32_t i)
{
	if(core->stack_values.count >= 1){
...
void OS::setCFuncClosure(int32_t offs, int32_t i)
{
	Core::Value func = core->getStackValue(offs);
	if(OS_VALUE_TYPE(func) == OS_VALUE_TYPE_CFUNCTION){
		OS_ASSERT(dynamic_cast<Core::GCCFunctionValue*>(OS_VALUE_VARIANT(func).cfunc));
...
void OS::setCFuncClosure(int32_t i)
{
	if(core->stack_values.count >= 2){
		Core::Value value = core->stack_values[core->stack_values.count - 2];
...
int32_t OS::getValueId(int32_t offs)
{
	Core::Value val = core->getStackValue(offs);
	if(OS_IS_VALUE_GC(val)){
...
OS::String OS::getValueName(int32_t offs)
{
	return core->getValueName(core->getStackValue(offs));
}

OS::String OS::getValueClassname(int32_t offs)
{
	return core->getValueClassname(core->getStackValue(offs));
}

OS::String OS::getValueNameOrClassname(int32_t offs)
{
...
#define OS_GET_PROP_VALUE_PTR(_result_bool, _result_value, _table_value, _index, _index_type, prototype_enabled) \
	do { \
		...
		int32_t local_index_type = (_index_type); \
		...
		if(local_table_value->type == OS_VALUE_TYPE_ARRAY && local_index_type == OS_VALUE_TYPE_NUMBER){ \
			...
			int32_t i; OS_NUMBER_TO_INT(i, OS_VALUE_NUMBER(local_index)); \
...
bool OS::Core::hasProperty(GCValue * table_value, Value index, bool getter_enabled, bool prototype_enabled)
{
	...
	if(OS_VALUE_TYPE(index) == OS_VALUE_TYPE_STRING){
		...
		int32_t size2 = OS_VALUE_VARIANT(index).string->getDataSize();
		{
			...
			int32_t size1 = strings->__issetAt.getDataSize();
		...
		}
		{
			...
			int32_t size1 = strings->__getAt.getDataSize();
			GCStringValue * getter_name = pushStringValue(buf1, size1, buf2, size2);
			if(getPropertyValue(value, table_value, getter_name, prototype_enabled)){
				pop(); // getter_name
				return true;
...
#define OS_GETTER_VALUE_PTR(_result_value, _this, _table_value, _index, _index_type, _getter_enabled, _prototype_enabled) \
	do { \
		...
		int32_t local3_index_type = (_index_type); \
		...
		if(local3_getter_enabled /*&& !hasSpecialPrefix(local3_index)*/){ \
			...
			if(local3_index_type == OS_VALUE_TYPE_STRING){ \
				...
				int32_t size2 = OS_VALUE_VARIANT(local3_index).string->getDataSize(); \
				GCStringValue * getter_name = pushStringValue(buf1, size1, buf2, size2); \
...
#define OS_GETTER_VALUE(_result_value, _table_value, _index, _index_type, _getter_enabled, _prototype_enabled) \
	do { \
		...
		int32_t local5_index_type = (_index_type); \
		OS_ASSERT(OS_VALUE_TYPE(local5_index) == (local5_index_type)); \
		const int32_t type = OS_VALUE_TYPE(local5_table_value); \
		if(type == OS_VALUE_TYPE_ARRAY && local5_index_type == OS_VALUE_TYPE_NUMBER){ \
			...
			int32_t i; OS_NUMBER_TO_INT(i, OS_VALUE_NUMBER(local5_index)); \
			if((i >= 0 || (i += arr->values.count) >= 0) && i < arr->values.count){ \
...
void OS::getProperty(int32_t offs, const OS_CHAR * name, bool getter_enabled, bool prototype_enabled)
{
	getProperty(offs, Core::String(this, name), getter_enabled, prototype_enabled);
}

void OS::getProperty(int32_t offs, const Core::String& name, bool getter_enabled, bool prototype_enabled)
{
	pushStackValue(offs);
...
void OS::Core::clearStackFunction(StackFunction * stack_func)
{
	if(--stack_func->locals->ref_count > 0){
		...
		int32_t count = func_decl->num_locals; // >opcode_stack_pos - stack_func->locals_stack_pos;
		if(count > 0){
...
void OS::Core::execute()
{
	...
	int32_t a, b, c, res, ret_stack_funcs = call_stack_funcs.count-1;
	Program * prog;
	Value * left_value, * right_value, * index_value, value;
	Locals * scope;
#ifdef OS_INFINITE_LOOP_OPCODES
	for(int32_t opcodes_executed = 0;; opcodes_executed++){
#else
	for(;;){
#endif
		...
		if(terminated){
			if(!terminated_exception.isNull() && !call_stack_overflow){
				for(;;){
					...
					FunctionDecl * func_decl = stack_func->func->func_decl;
					int32_t num_try_blocks = func_decl->num_try_blocks;
					int32_t cur_opcode_pos = (int32_t)(stack_func->opcodes - stack_func->func->prog->opcodes.buf);
					FunctionDecl::TryBlock * found_try_block = NULL;
					for(int32_t i = 0; i < num_try_blocks; i++){
					...
					// leave function
					int32_t need_ret_values = stack_func->need_ret_values;
...
#else
		uint32_t opcode = OS_GET_OPCODE_WITH_CC(instruction);
#endif
#if 0
		{
			prog = stack_func->func->prog;
			int32_t opcode_pos = stack_func->opcodes - prog->opcodes.buf;
		...
		OS_CASE_OPCODE_ALL(OP_LOGIC_BOOL):
			{
				...
				res = (int32_t)valueToBool(stack_func_locals[a]) ^ OS_GETARG_B(instruction);
...
#else
				res = (int32_t)isEqualExactly(*left_value, *right_value) ^ OS_GETARG_B(instruction);
#endif
				if(!(OS_GETARG_C(instruction))){
		...
		OS_CASE_OPCODE_ALL(OP_LOGIC_EQ):
			{
				...
				if(OS_IS_VALUE_NUMBER(*left_value) && OS_IS_VALUE_NUMBER(*right_value)){
					res = (int32_t)(OS_VALUE_NUMBER(*left_value) == OS_VALUE_NUMBER(*right_value)) ^ b;
		...
		case OP_TAIL_CALL:
			{
				...
				OS_ASSERT(call_stack_funcs.count > 0 && &call_stack_funcs[call_stack_funcs.count-1] == stack_func);
				int32_t caller_stack_size = stack_func->caller_stack_size;
				...
				int32_t call_stack_funcs_size = call_stack_funcs.count;
				...
				if(stack_func->self_for_proto && func_value->name){
					...
					if(proto){
						...
						for(GCValue * cur_proto = proto;;){
							...
							if(!cur_proto){
								struct Lib {
									static int32_t setProperty(OS * os, int32_t params, int32_t, int32_t, void*)
									{
										if(params != 2){
									...
									static int32_t deleteProperty(OS * os, int32_t params, int32_t, int32_t, void*)
									{
										if(params != 2){
											os->setException("two arguments required");
										}else{
...
void OS::runOp(OS_EOpcode opcode)
{
	struct Lib
	{
		...
		{
			int32_t count = core->stack_values.count;
			if(count < 2){
				core->pushNull();
				return false;
			}
		...
		{
			int32_t offs = core->allocator->getAbsoluteOffs(-2);
...
int32_t OS::getLen(int32_t offs)
{
...
void OS::setFunc(const FuncDef& def, bool setter_enabled, int32_t closure_values, void * user_param)
{
	const FuncDef list[] = {def, {}};
	setFuncs(list, setter_enabled, closure_values, user_param);
}

void OS::setFuncs(const FuncDef * list, bool setter_enabled, int32_t closure_values, void * user_param)
{
	if(list)
	for(; list->func; list++){
		pushStackValue(-1);
		...
		for(int32_t i = 0; i < closure_values; i++){
...
bool OS::nextIteratorStep(int32_t results, const Core::String& iter_func)
{
...
bool OS::nextIteratorStep(int32_t results)
{
...
void OS::initCoreFunctions()
{
	struct Format
	{
		...
		static int32_t getLen(OS * os, const String& s)
		{
			os->pushString(s);
			os->runOp(OP_LENGTH);
			return os->popInt();
		...
		static void number(OS::Core::Buffer& buf, OS_INT num, int32_t base, int32_t size, int32_t precision, int32_t type)
		{
		...
		static void flt(OS::Core::Buffer& buf, double num, int32_t size, int32_t precision, OS_CHAR fmt, int32_t flags)
		{
			...
			if(precision < 0){
				double p = 10.0;
				for(int32_t j = -precision-1; j > 0; j--){
					p *= 10.0;
				}
		...
		static int32_t sprintf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
	...
	struct Lib
	{
		static int32_t print(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
				int32_t offs = os->getAbsoluteOffs(-params);
		...
		static int32_t echo(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			for(int32_t i = 0; i < params; i++){
		...
		static int32_t printf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
		...
		static int32_t concat(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params == 2){
		...
		static int32_t compileText(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			if(params < 1){
				return 0;
			}
		...
		static int32_t compileFile(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			if(params < 1){
				return 0;
			}
		...
		static int32_t compileFakeFile(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			if(params < 2){
				return 0;
			}
		...
		static int32_t resolvePath(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params >= 1){
		...
		static int32_t debugBackTrace(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			switch(params){
			case 0:
		...
		static int32_t terminate(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->setTerminated(true, os->toInt(-params));
			return 0;
		}

		static int32_t deleteOp(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			OS_ASSERT(params == 2);
		...
		static int32_t length(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			OS_ASSERT(params == 1);
			Core::Value func;
		...
		static int32_t extends(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			OS_ASSERT(params == 2);
		...
		static int32_t in(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t is(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params != 2) return 0;
			os->pushBool(os->is());
			return 1;
		...
		static int32_t as(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params == 2){
		...
		static int32_t typeOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t booleanOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t numberOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t stringOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t arrayOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t objectOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t userdataOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t functionOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t toBoolean(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t parseInt(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
				OS::String buf = os->toString(-params+0);
				int32_t radix = params > 1 ? os->toInt(-params+1) : 10;
		...
		static int32_t parseFloat(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
				OS::String buf = os->toString(-params+0);
				const OS_CHAR * str = buf.toChar();
		...
		static int32_t toNumber(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params >= 3){
				int32_t radix = os->toInt(-params+1);
		...
		static int32_t toString(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1) return 0;
		...
		static int32_t getFilename(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core * core = os->core;
			for(int32_t i = core->call_stack_funcs.count-1; i >= 0; i--){
				Core::StackFunction * stack_func = core->call_stack_funcs.buf + i;
		...
		static int32_t getDirname(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			int32_t r = getFilename(os, params, 0, 0, NULL);
			if(r > 0){
				OS_ASSERT(r == 1);
		...
		static int32_t getVersion(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushString(OS_VERSION);
			return 1;
		}

		static int32_t construct(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			// TODO: correct?
		...
		static int32_t initNewInstance(OS * os, int32_t params, int32_t, int32_t, void*)
		{
...
{
	static int32_t iterator_crc = OS_PTR_HASH(&iterator_crc);

	struct Object
	{
		static int32_t getProperty(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t setSmartProperty(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			bool setter_enabled = false;
			switch(params){
			case 0:
		...
		static int32_t getValueId(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->getValueId(-params-1));
			return 1;
		}

		static int32_t getValueName(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t getValueClassname(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushString(os->core->getValueClassname(os->core->getStackValue(-params-1)));
			return 1;
		}

		static int32_t iteratorStep(OS * os, int32_t params, int32_t closure_values, int32_t, void*)
		{
		...
		static int32_t iterator(OS * os, int32_t params, bool ascending)
		{
		...
		static int32_t iterator(OS * os, int32_t params, int32_t closure_values, int32_t, void*)
		{
			return iterator(os, params + closure_values, true);
		}

		static int32_t reverseIterator(OS * os, int32_t params, int32_t closure_values, int32_t, void*)
		{
			return iterator(os, params + closure_values, false);
		}

		static int32_t smartSort(OS * os, int32_t params, 
			int32_t(*arrcomp)(OS*, const void*, const void*, void*), 
			int32_t(*objcomp)(OS*, const void*, const void*, void*), void * user_param = NULL)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
		...
		static int32_t sort(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1){
		...
		static int32_t length(OS * os, int32_t params, int32_t closure_values, int32_t, void*)
		{
		...
		static int32_t toJson(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			...
			case OS_VALUE_TYPE_ARRAY:
				{
					...
					buf += OS_TEXT("[");
					for(int32_t i = 0; i < arr->values.count && !os->isExceptionSet(); i++){
						if(i > 0){
							buf += OS_TEXT(",");
						}						
			...
			case OS_VALUE_TYPE_OBJECT:
				{
					...
					for(int32_t i = 0; prop && !os->isExceptionSet(); prop = prop->next, i++){
						if(i > 0){
							buf += OS_TEXT(",");
						}
		...
		static int32_t valueOf(OS * os, int32_t params, int32_t closure_values, int32_t, void*)
		{
...
dump_object:
					...
					for(int32_t i = 0; prop && !os->isExceptionSet(); prop = prop->next, i++){
						...
						if(OS_VALUE_TYPE(prop->index) == OS_VALUE_TYPE_NUMBER){
							...
							need_index = (int32_t)(OS_VALUE_NUMBER(prop->index) + 1);
		...
		static int32_t push(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			Core::Value value = os->core->getStackValue(-params);
			OS_INT num_index = 0;
		...
		static int32_t pop(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t unshift(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t shift(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t hasOwnProperty(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t hasProperty(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::GCValue * self = os->core->getStackValue(-params-1).getGCValue();
			if(self){
				if(params > 0){
		...
		static int32_t sub(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			int32_t start, len, size;
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
			case OS_VALUE_TYPE_USERDATA:
		...
		/* static int32_t merge(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1){
				OS_ASSERT(params == 0);
				// return this
					...
					case OS_VALUE_TYPE_ARRAY:
						{
							...
							for(int32_t j = 0; j < OS_VALUE_VARIANT(value).arr->values.count; j++){
		...
		static int32_t getKeys(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			...
			case OS_VALUE_TYPE_CFUNCTION:
				if(OS_VALUE_VARIANT(value).object->table){
					...
					Core::Property * prop = OS_VALUE_VARIANT(value).object->table->first;
					for(int32_t i = 0; prop; prop = prop->next, i++){
		...
		static int32_t getValues(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value value = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(value)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t join(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Buffer buf(os);
		...
		static int32_t clear(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t clone(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value val = os->core->getStackValue(-params-1);
		...
		static int32_t unpack(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			Core::GCValue * value = os->core->getStackValue(-params-1).getGCValue();
		...
		static int32_t getFirst(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t setFirst(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t deleteFirst(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t getLast(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t setLast(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t deleteLast(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_OBJECT:
		...
		static int32_t __construct(OS * os, int32_t params, int32_t, int32_t, void*)
		{
...
int32_t OS::Core::prototypeFunctionApply(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
{
	int32_t offs = os->getAbsoluteOffs(-params);
	...
	if(OS_VALUE_TYPE(params_var) == OS_VALUE_TYPE_ARRAY){
		int32_t count = OS_VALUE_VARIANT(params_var).arr->values.count;
		for(int32_t i = 0; i < count; i++){
	...
	if(OS_VALUE_TYPE(params_var) == OS_VALUE_TYPE_OBJECT){ // && OS_VALUE_VARIANT(params_var).object->table){
		int32_t func_type = os->getType(offs-1);
		...
		if(func){
			...
			int32_t num_params = func_decl->num_params;
			Core::Value value;
			Core::GCValue * table_value = OS_VALUE_VARIANT(params_var).value;
			for(int32_t i = Core::PRE_VARS; i < num_params; i++){ // skip func & this
...
int32_t OS::Core::prototypeFunctionCall(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
{
...
{
	static int32_t array_iterator_crc = OS_PTR_HASH(&array_iterator_crc);
	...
	struct Array
	{
		...
		static int32_t arrayIteratorStep(OS * os, int32_t params, int32_t closure_values, int32_t, void*)
		{
			OS_ASSERT(closure_values == 2);
			Core::Value self_var = os->core->getStackValue(-closure_values + 0);
			int32_t * pi = (int32_t*)os->toUserdata(array_iterator_crc, -closure_values + 1);
		...
		static int32_t iterator(OS * os, int32_t params, EIterType iter_type)
		{
			...
			if(OS_VALUE_TYPE(self_var) == OS_VALUE_TYPE_ARRAY){
				...
				int32_t * pi = (int32_t*)os->pushUserdata(array_iterator_crc, sizeof(int32_t)*2);
				OS_ASSERT(pi);
		...
		static int32_t dumpIterator(OS * os, int32_t params, int32_t closure_values, int32_t, void*)
		{
		...
		static int32_t indexOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			Core::Value value = os->core->getStackValue(-params);
			if(OS_VALUE_TYPE(self_var) == OS_VALUE_TYPE_ARRAY){
				...
				Core::GCArrayValue * arr = OS_VALUE_VARIANT(self_var).arr;
				int32_t count = arr->values.count;
				int32_t i = params >= 2 ? os->toInt(-params+1) : 0;
				if(i < 0 && (i += count) < 0){
					i = 0;
				}
				for(; i < count; i++){
		...
		static int32_t lastIndexOf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			Core::Value value = os->core->getStackValue(-params);
			if(OS_VALUE_TYPE(self_var) == OS_VALUE_TYPE_ARRAY){
				...
				Core::GCArrayValue * arr = OS_VALUE_VARIANT(self_var).arr;
				int32_t count = arr->values.count;
				int32_t i = params >= 2 ? os->toInt(-params+1) : count-1;
				if(i < 0 && (i += count) < 0){
					i = 0;
				}else if(i > count-1){
					i = count-1;
		...
		static int32_t join(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			...
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_ARRAY:
				for(int32_t i = 0; i < OS_VALUE_VARIANT(self_var).arr->values.count; i++){
					if(i > 0){
						buf.append(str);
					}
			...
			case OS_VALUE_TYPE_ARRAY:
				{
					...
					for(int32_t i = 0; i < OS_VALUE_VARIANT(value).arr->values.count; i++){
		...
		static int32_t getFirst(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_ARRAY:
		...
		static int32_t setFirst(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_ARRAY:
		...
		static int32_t deleteFirst(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_ARRAY:
		...
		static int32_t getLast(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_ARRAY:
		...
		static int32_t setLast(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_ARRAY:
		...
		static int32_t deleteLast(OS * os, int32_t params, int32_t, int32_t need_ret_values, void*)
		{
			Core::Value self_var = os->core->getStackValue(-params-1);
			switch(OS_VALUE_TYPE(self_var)){
			case OS_VALUE_TYPE_ARRAY:
				if(OS_VALUE_VARIANT(self_var).arr->values.count > 0){
					int32_t last = OS_VALUE_VARIANT(self_var).arr->values.count-1;
...
void OS::initBufferClass()
{
	struct Lib
	{
		static int32_t __newinstance(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
		...
		static int32_t append(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
		...
		static int32_t clear(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
			OS_GET_SELF(Core::Buffer*);
			self->clear();
			CtypeValue<Core::Buffer*>::push(os, self);
		...
		static int32_t valueOf(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
			if(os->isUserdata(CtypeId<Core::Buffer>::getId(), -params-1)){
				int32_t offs = os->getAbsoluteOffs(-params-1);
		...
		static int32_t len(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
			OS_GET_SELF(Core::Buffer*);
...
static int32_t machine_little_endian;

/* Mapping of byte from char (8bit) to int32_t for machine endian */
static int32_t byte_map[1];

/* Mappings of bytes from int32_t (machine dependant) to int32_t for machine endian */
static int32_t int_map[sizeof(int32_t)];

/* Mappings of bytes from shorts (16bit) for all endian environments */
static int32_t machine_endian_short_map[2];
static int32_t big_endian_short_map[2];
static int32_t little_endian_short_map[2];

/* Mappings of bytes from longs (32bit) for all endian environments */
static int32_t machine_endian_long_map[4];
static int32_t big_endian_long_map[4];
static int32_t little_endian_long_map[4];

void OS::initStringClass()
{
	struct PackLib
	{
		static void packLong(int32_t val, int32_t size, int32_t *map, char *output)
		{
			char *v = (char*)&val;
		 ...
		 */
		static int32_t pack(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			int32_t offs = os->getAbsoluteOffs(-(++params));
			
			OS::String format_str = os->toString(offs);
			const OS_CHAR * format = format_str.toChar();
			int32_t formatlen = format_str.getLen();

			/* We have a maximum of <formatlen> format codes to deal with */
			char * formatcodes = (char*)os->malloc(formatlen * sizeof(*formatcodes) OS_DBG_FILEPOS);
			int32_t * formatargs = (int32_t*)os->malloc(formatlen * sizeof(*formatargs) OS_DBG_FILEPOS);
			int32_t i, currentarg = 1;
...
#define OS_INC_OUTPUTPOS(a,b) \
	if ((a) < 0 || ((INT_MAX - outputpos)/((int32_t)b)) < (a)) { \
	...
	outputpos += (a)*(b);
			...
			int32_t outputpos = 0, outputsize = 0;
			for (i = 0; i < formatcount; i++) {
				char code = formatcodes[i];
				int32_t arg = formatargs[i];
				...
				switch (code) {
					...
					case 'i': 
					case 'I':
						OS_INC_OUTPUTPOS(arg, sizeof(int32_t))
						break;
			...
			for (i = 0; i < formatcount; i++) {
				...
				switch (code) {
					...
					case 'h': 
					case 'H': {
						int32_t nibbleshift = (code == 'h') ? 0 : 4;
						int32_t first = 1;
					...
					case 'C':
						while (arg-- > 0) {
							packLong((int32_t)os->toNumber(offs + currentarg++), 1, byte_map, &output[outputpos]);
							outputpos++;
						}
						break;
					...
					case 'n': 
					case 'v': {
						int32_t *map = machine_endian_short_map;
						if (code == 'n') {
						...
						while (arg-- > 0) {
							packLong((int32_t)os->toNumber(offs + currentarg++), 2, map, &output[outputpos]);
							outputpos += 2;
						}
						break;
					}
					...
					case 'I': 
						while (arg-- > 0) {
							packLong((int32_t)os->toNumber(offs + currentarg++), sizeof(int32_t), int_map, &output[outputpos]);
					...
					case 'N': 
					case 'V': {
						int32_t *map = machine_endian_long_map;
						if (code == 'N') {
						...
						while (arg-- > 0) {
							packLong((int32_t)os->toNumber(offs + currentarg++), 4, map, &output[outputpos]);
							outputpos += 4;
						}
						break;
					}
		...
		static int32_t unpackLong(const char *data, int32_t size, int32_t issigned, int32_t *map)
		{
			int32_t result = issigned ? -1 : 0;
		 ...
		 * Implemented formats are A, a, h, H, c, C, s, S, i, I, l, L, n, N, f, d, x, X, @.
		 */
		static int32_t unpack(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1){
				return 0;
			}
			...
			int32_t inputlen = input_str.getLen();
			int32_t inputpos = 0, ret_values = 0;
			...
			while (formatlen-- > 0) {
				...
				switch (type) {
					...
					/* Use sizeof(int32_t) bytes of input */
					case 'i': 
					case 'I':
						size = sizeof(int32_t);
						break;
				...
				for (int32_t i = 0; i != arg; i++ ) {
					...
					if ((inputpos + size) <= inputlen) {
						switch ((int32_t) type) {
							...
							case 'A': {
								char pad = (type == 'a') ? '\0' : ' ';
								int32_t len = inputlen - inputpos;	/* Remaining string */
							...
							case 'h': 
							case 'H': {
								int32_t len = (inputlen - inputpos) * 2;	/* Remaining */
								int32_t nibbleshift = (type == 'h') ? 0 : 4;
								int32_t first = 1;
								char *buf;
								int32_t ipos, opos;
							...
							case 'c': 
							case 'C': {
								int32_t issigned = (type == 'c') ? (input[inputpos] & 0x80) : 0;
								int32_t v = unpackLong(&input[inputpos], 1, issigned, byte_map);
								ret_values++;
								os->pushNumber(v);
							...
							case 'I': {
								...
								if (type == 'i') {
									issigned = input[inputpos + (machine_little_endian ? (sizeof(int32_t) - 1) : 0)] & 0x80;
								}

								v = unpackLong(&input[inputpos], sizeof(int32_t), issigned, int_map);
								ret_values++;
								os->pushNumber(v);
								// add_assoc_long(return_value, n, v);
								break;
		...
		static void init()
		{
			int32_t machine_endian_check = 1;
			...
			if (machine_little_endian) {
				...
				OS_ASSERT(!IS_LITTLE_ENDIAN);
				// zval val;
				// int32_t size = sizeof(Z_LVAL(val));
				...
				byte_map[0] = size - 1;
				for (int32_t i = 0; i < (int32_t)sizeof(int32_t); i++) {
					int_map[i] = size - (sizeof(int32_t) - i);
				}
	...
	struct String
	{
		...
		static int32_t lengthUtf8(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			int32_t len = 0;
		...
		static int32_t subUtf8(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			int32_t start, len;
			OS::String str = os->toString(-params-1);
			...
			size = (int32_t)(cur_end - string);
			os->pushString((void*)string, size);
			return 1;
		}

		static int32_t replace(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			OS::String subject = os->toString(-params-1);
			int32_t subject_len = subject.getLen();
			if(params >= 1){
				...
				if(search_len > 0 && search_len <= subject_len){
					...
					bool found = false;
					int32_t start = 0;
					for(int32_t i = 0; i < subject_len-search_len+1;){
		...
		static int32_t find(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params >= 1){
				OS::String subject = os->toString(-params-1);
				int32_t subject_len = subject.getLen();
				...
				if(search_len > 0 && search_len <= subject_len && i >= 0){
					const OS_CHAR * subject_str = subject.toChar();
					const OS_CHAR * search_str = search.toChar();
					int32_t end = subject_len - search_len;
					for(; i <= end; i++){
						if(OS_MEMCMP(subject_str + i, search_str, sizeof(OS_CHAR)*search_len) == 0){
							os->pushNumber(i);
							return 1;
		...
		static int32_t findUtf8(OS * os, int32_t params, int32_t closure_values, int32_t need_ret_values, void * user_param)
		{
			int32_t offs = os->getAbsoluteOffs(-params-1);
			...
			int32_t ret = find(os, params, closure_values, need_ret_values, user_param);
			if(ret > 0){
				int32_t pos = os->toInt(-ret);
				int32_t subject_len = subject.getLen();
		...
		static int32_t compare(OS * os, int32_t left_offs, int32_t right_offs)
		{
#if 1
			OS::String left(os);
		...
		static int32_t cmp(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
		...
		static int32_t rcmp(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
		...
		static int32_t trim(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t upper(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			OS::String self = os->toString(-params - 1); // this
			const OS_CHAR * str = self.toChar();
			int32_t len = self.getLen();
			for(int32_t i = 0; i < len; i++){
		...
		static int32_t lower(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			OS::String self = os->toString(-params - 1); // this
			const OS_CHAR * str = self.toChar();
			int32_t len = self.getLen();
		...
		static int32_t split(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			int32_t offs = os->getAbsoluteOffs(-params-1);
			...
			if(params >= 1){
				...
				if(search_len > 0 && search_len <= subject_len){
					...
					int32_t max_count = params >= 2 ? os->toInt(offs+2) : INT_MAX;
					if(max_count > 1){
						bool found = false;
						int32_t start = 0;
						for(int32_t i = 0; i < subject_len-search_len+1;){
...
int32_t OS::Core::File::getSize() const
{
	return os->getFileSize(f);
}

int32_t OS::Core::File::getPos() const
{
	return os->seekFile(f, 0, SEEK_CUR);
}

void OS::Core::File::setPos(int32_t pos)
{
...
OS::Core::String OS::Core::File::read(int32_t len)
{
	if(!f || len <= 0){
	...
	int32_t read_len = os->readFile(buf.buffer.buf, len, f);
	if(read_len == 0){
		// TODO:: error ?!
	}
...
int32_t OS::Core::File::write(const void * data, int32_t len)
{
	return os->writeFile(data, len, f);
}

int32_t OS::Core::File::write(const Core::String& str)
{
...
void OS::initFileClass()
{
	struct Lib
	{
		...
		static int32_t open(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
		...
		static int32_t read(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
			OS_GET_SELF(Core::File*);
			if(params > 0){
		...
		static int32_t write(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
			OS_GET_SELF(Core::File*);
			if(params >= 2){
				String str = os->toString(-params);
				int32_t size = os->toInt(-params+1);
		...
		static int32_t existsFunc(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
...
void OS::initExceptionClass()
{
	struct Lib
	{
		static int32_t __construct(OS * os, int32_t params, int32_t, int32_t, void * user_param)
		{
			int32_t offs = os->getAbsoluteOffs(-params - 1);
			if(params >= 1){
...
void OS::initFunctionClass()
{
	struct Function
	{
		...
		static int32_t applyEnv(OS * os, int32_t params, int32_t, int32_t need_ret_values, void *)
		{
			Core::Value save_env;
			...
			os->remove(-params);
			int32_t r = Core::prototypeFunctionApply(os, params-1, 0, need_ret_values, NULL);
			if(OS_VALUE_TYPE(func) == OS_VALUE_TYPE_FUNCTION){
		...
		static int32_t callEnv(OS * os, int32_t params, int32_t, int32_t need_ret_values, void *)
		{
			Core::Value save_env;
			Core::Value func = os->core->getStackValue(-params-1);
			if(OS_VALUE_TYPE(func) == OS_VALUE_TYPE_FUNCTION){
			...
			os->remove(-params);
			int32_t r = Core::prototypeFunctionCall(os, params-1, 0, need_ret_values, NULL);
			if(OS_VALUE_TYPE(func) == OS_VALUE_TYPE_FUNCTION){
				OS_VALUE_VARIANT(func).func->env = save_env;
			}
			return r;
...
#define OS_RAND_RANGE(__n, __min, __max, __tmax) \
	(__n) = (__min) + (int32_t) ((double) ( (double) (__max) - (__min) + 1.0) * ((__n) / ((__tmax) + 1.0)))

#if defined _MSC_VER && !defined IW_SDK
#include <windows.h>
#define OS_RAND_GENERATE_SEED() (((int32_t) (time(0) * GetCurrentProcessId())) ^ ((int32_t) (1000000.0)))
// #elif !defined IW_SDK
// #define OS_RAND_GENERATE_SEED() (((int32_t) (time(0) * getpid())) ^ ((int32_t) (1000000.0)))
#else
#define OS_RAND_GENERATE_SEED() (((int32_t) (time(0))) ^ ((int32_t) (1000000.0)))
#endif 

int32_t OS::Core::getRandSeed()
{
	return rand_seed;
}

void OS::Core::setRandSeed(int32_t seed)
{
...
int32_t OS::getRandSeed()
{
	return core->getRandSeed();
}

void OS::setRandSeed(int32_t seed)
{
...
void OS::initMathModule()
{
	struct Math
	{
		static int32_t minmax(OS * os, int32_t params, OS_EOpcode opcode)
		{
			OS_ASSERT(params >= 0);
			if(params <= 1){
				return params;
			}
			int32_t params_offs = os->getAbsoluteOffs(-params);
			os->pushStackValue(params_offs); // save temp result
			for(int32_t i = 1; i < params; i++){
		...
		static int32_t min_func(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			return minmax(os, params, OP_LOGIC_LE);
		}

		static int32_t max_func(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			return minmax(os, params, OP_LOGIC_GE);
		}

		static double iabs(double p)
		{
			return ::fabs(p);
		}
		...
		static int32_t frexp(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(!params) return 0;
			int32_t e;
		...
		static double ldexp(double x, int32_t y)
		{
			return ::ldexp(x, y);
		}
		...
		static int32_t random(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t getRandomSeed(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber((OS_NUMBER)os->core->getRandSeed());
			return 1;
		}

		static int32_t setRandomSeed(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
				os->core->setRandSeed((int32_t)os->toNumber(-params));
			}
			return 0;
		}
		...
		static int32_t modf(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(!params) return 0;
			double ip;
		...
		static int32_t log(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(!params) return 0;
...
void OS::initJsonModule()
{
	struct Lib 
	{
		static int32_t encode(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
				int32_t offs = os->getAbsoluteOffs(-params+0);
				os->getGlobal(OS_TEXT("Object"));
		...
		static int32_t decode(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
				int32_t ret = 0;
...
void OS::initGCModule()
{
	struct GC
	{
		static int32_t getAllocatedBytes(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->getAllocatedBytes());
			return 1;
		}
		static int32_t getMaxAllocatedBytes(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->getMaxAllocatedBytes());
			return 1;
		}
		static int32_t getUsedBytes(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->getUsedBytes());
			return 1;
		}
		static int32_t getCachedBytes(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->getCachedBytes());
			return 1;
		}
		static int32_t getNumObjects(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->core->values.count);
			return 1;
		}
		static int32_t getNumCreatedObjects(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->core->num_created_values);
			return 1;
		}
		static int32_t getNumDestroyedObjects(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->core->num_destroyed_values);
			return 1;
		}
		static int32_t getStartWhenUsedBytes(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(os->getGCStartWhenUsedBytes());
			return 1;
		}
		static int32_t setStartWhenUsedBytes(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params > 0){
		...
		static int32_t getValueBytes(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->pushNumber(sizeof(Core::Value));
			return 1;
		}
		static int32_t getNanTrickUsed(OS * os, int32_t params, int32_t, int32_t, void*)
		{
		...
		static int32_t full(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			os->gcFull();
			return 0;
		}
...
void OS::initLangTokenizerModule()
{
	struct LangTokenizer
	{
		...
		static int32_t getTokenType(Core::TokenType type)
		{
			switch(type){
		...
		{
			os->newArray();
			int32_t count = tokenizer.getNumTokens();
			for(int32_t i = 0; i < count; i++){
				os->pushStackValue(-1);
				os->newObject();
				{
					...
					int32_t type = getTokenType(token->type);
		...
		static int32_t parseText(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1){
				return 0;
			}
		...
		static int32_t parseFile(OS * os, int32_t params, int32_t, int32_t, void*)
		{
			if(params < 1){
				return 0;
			}
...
{
	if(!stack_func->arguments){
		int32_t num_params = stack_func->num_params;
...
void OS::Core::pushBackTrace(int32_t skip_funcs, int32_t max_trace_funcs)
{
	...
	for(int32_t i = call_stack_funcs.count-1-skip_funcs; i >= 0 && arr->values.count < max_trace_funcs; i--){
		...
		if(prog->filename.getDataSize() && prog->debug_info.count > 0){
			int32_t opcode_pos = (int32_t)(stack_func->opcodes - prog->opcodes.buf); // .getPos() + stack_func->func->func_decl->opcodes_pos;
			debug_info = prog->getDebugInfo(opcode_pos);
		}
		setPropertyValue(obj, line_str, debug_info ? (int32_t)debug_info->line : Value(), false);
		setPropertyValue(obj, pos_str, debug_info ? (int32_t)debug_info->pos : Value(), false);
...
void OS::Core::callFT(int32_t start_pos, int32_t call_params, int32_t ret_values, GCValue * self_for_proto, OS_ECallEnter call_enter, OS_ECallType call_type, OS_ECallThisUsage call_this_usage)
{
		...
		case OS_VALUE_TYPE_FUNCTION:
			{
				...
				if(call_params > func_decl->num_params){
					int32_t num_extra_params = call_params - func_decl->num_params;
				...
				int32_t clear_values = func_decl->stack_size - call_params;
				if(clear_values > 0){
		...
		case OS_VALUE_TYPE_CFUNCTION:
			{
				...
				int32_t save_stack_size = stack_values.count;
...
void OS::Core::callFT(int32_t params, int32_t ret_values, GCValue * self_for_proto, OS_ECallEnter call_enter, OS_ECallType call_type, OS_ECallThisUsage call_this_usage)
{
	params += 2;
	int32_t start_pos = stack_values.count - params;
...
void OS::Core::callFT(int32_t params, int32_t ret_values, OS_ECallType call_type, OS_ECallThisUsage call_this_usage)
{
	params += 2;
	int32_t start_pos = stack_values.count - params;
...
void OS::Core::callTF(int32_t params, int32_t ret_values, OS_ECallType call_type, OS_ECallThisUsage call_this_usage)
{
	params += 2;
	int32_t start_pos = stack_values.count - params;
	GCValue * self_for_proto = NULL;
...
void OS::Core::callF(int32_t params, int32_t ret_values, OS_ECallType call_type)
{
	 // insert null as 'this' value to get stack values properly
	if(params > 0){
#if 1
		int32_t offs = stack_values.count - params;

		// reserveStackValues(stack_values.count+1);
		int32_t new_count = stack_values.count + 1;
...
/* void OS::call(int32_t params, int32_t ret_values, OS_ECallType call_type, OS_ECallThisUsage call_this_usage)
{
	core->call(params, ret_values, call_type, call_this_usage);
} */

void OS::callFT(int32_t params, int32_t ret_values, OS_ECallType call_type, OS_ECallThisUsage call_this_usage)
{
	core->callFT(params, ret_values, call_type, call_this_usage);
}

void OS::callTF(int32_t params, int32_t ret_values, OS_ECallType call_type, OS_ECallThisUsage call_this_usage)
{
	core->callTF(params, ret_values, call_type, call_this_usage);
}

void OS::callF(int32_t params, int32_t ret_values, OS_ECallType call_type)
{
	core->callF(params, ret_values, call_type);
}

void OS::eval(const OS_CHAR * str, int32_t params, int32_t ret_values, OS_ESourceCodeType source_code_type, bool check_utf8_bom, bool handle_exception)
{
	eval(String(this, str), params, ret_values, source_code_type, check_utf8_bom, handle_exception);
}

void OS::eval(const String& str, int32_t params, int32_t ret_values, OS_ESourceCodeType source_code_type, bool check_utf8_bom, bool handle_exception)
{
	resetException();
...
void OS::evalFakeFile(const OS_CHAR * filename, const OS_CHAR * str, int32_t params, int32_t ret_values, OS_ESourceCodeType source_code_type, bool check_utf8_bom, bool handle_exception)
{
	evalFakeFile(String(this, filename), String(this, str), params, ret_values, source_code_type, check_utf8_bom, handle_exception);
}

void OS::evalFakeFile(const String& filename, const String& str, int32_t params, int32_t ret_values, OS_ESourceCodeType source_code_type, bool check_utf8_bom, bool handle_exception)
{
	resetException();
...
void OS::evalProtected(const OS_CHAR * str, int32_t params, int32_t ret_values, OS_ESourceCodeType source_code_type, bool check_utf8_bom, bool handle_exception)
{
	resetException();
...
void OS::require(const OS_CHAR * filename, bool required, int32_t ret_values, OS_ESourceCodeType source_code_type, bool check_utf8_bom, bool handle_exception)
{
	require(String(this, filename), required, ret_values, source_code_type, check_utf8_bom, handle_exception);
}

void OS::require(const String& filename, bool required, int32_t ret_values, OS_ESourceCodeType source_code_type, bool check_utf8_bom, bool handle_exception)
{
	resetException();
...
int32_t OS::getSetting(OS_ESettings setting)
{
	switch(setting){
...
int32_t OS::setSetting(OS_ESettings setting, int32_t value)
{
	struct Lib {
		static int32_t ret(bool& cur_value, int32_t new_value)
		{
			int32_t old = cur_value;
...
static void qsortShortsort(OS * os, char *lo, char *hi, unsigned width, int32_t (*comp)(OS*, const void *, const void *, void*), void * user_params)
{
	char *p, *max;
...
void OS::qsort(void *base, unsigned num, unsigned width, int32_t (*comp)(OS*, const void *, const void *, void*), void * user_params)
{
	char *lo, *hi;
	char *mid;
	char *l, *h;
