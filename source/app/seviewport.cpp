...
void ViewportRotate(int which, int dx, int dy)
{
	...
	if(MAG_VEC3F(view - t->m_up) <= EPSILON || MAG_VEC3F(Vec3f(0,0,0) - view - t->m_up) <= EPSILON)
	{
	strafe = Vec3f(1,0,0);
...
void DragV(Brush* b, BrushSide* s, int j, Vec3f& newv, bool& mergedv, bool* invalidv, bool& remove)
{
	...
	else
	{
		...
		if(MAG_VEC3F( norm - s->m_plane.m_normal ) > MAG_VEC3F( Vec3f(0,0,0) - norm - s->m_plane.m_normal ))
		{
