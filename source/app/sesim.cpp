...
void DrawOutlines(EdMap* map, std::list<ModelHolder>& modelholder)
{
	...
	for(std::list<Brush>::iterator b=map->m_brush.begin(); b!=map->m_brush.end(); b++)
	{
		for(int i=0; i<b->m_nsides; i++)
		{
			...
			float alpha = 1.0f - Dot(side->m_centroid - g_cam.m_view) / (PROJ_RIGHT*2.0f/g_zoom) / (PROJ_RIGHT*2.0f/g_zoom);

			glUniform4f(shader->slot[SSLOT_COLOR], 0.2f, 0.9f, 0.3f, alpha);

			//glVertexAttribPointer(shader->slot[SSLOT_POSITION], 3, GL_FLOAT, GL_FALSE, 0, side->m_outline.m_drawoutva);
	...
	for(std::list<ModelHolder>::iterator mhiter = modelholder.begin(); mhiter != modelholder.end(); mhiter++)
	{
		...
		float alpha = 1.0f - Dot((pmh->absmin+pmh->absmax)/2.0f - g_cam.m_view) / (PROJ_RIGHT*2.0f/g_zoom) / (PROJ_RIGHT*2.0f/g_zoom);
...
void SelectDrag_VertFaceBrush(EdMap* map, Matrix* mvp, int w, int h, int x, int y, Vec3f eye, bool persp)
{
	...
	for(int i=0; i<g_selB.size(); i++)
	{
		...
		for(int j=0; j<b->m_nsharedv; j++)
		{
			...
			{
				float thisD = MAG_VEC3F(b->m_sharedv[j] - eye);
		...
		for(int j=0; j<b->m_nsides; j++)
		{
			...
			{
				float thisD = MAG_VEC3F(side->m_centroid - eye);
