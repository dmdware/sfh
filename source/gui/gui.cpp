...
#ifdef USEZOOM
uint32_t g_zoomtex = -1;
bool g_zoomdrawframe = false;
uint32_t g_zoomfbo = -1;
#endif
...
void GUI::draw()
{
	...
	if(si >= 0)
	{
		...
		si = si%(ut->nframes);
		uint32_t spi = ut->sprite[si/ut->nframes][si%ut->nframes];
		//uint32_t spi = ut->sprite[0][si%ut->nframes];
	...
	DrawImage(difftex->texname, 0, 0, 150, 150, 0, 0, 1, 1, m_crop);
...
void SetStatus(const char* status, bool logthis)
{
	if(logthis)
	{
		Log(status);
