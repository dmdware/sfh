...
void LoadFont(int32_t index, const char* fontfile)
{
	...
	char fullfontpath[WF_MAX_PATH+1];
...
void AddGlyph(BmpFont* f, uint32_t g)
{

}

void CheckGlyph(BmpFont* f, uint32_t g)
{
	Glyph* pg =& f->glyph[g];
	if(!pg->rend)
		AddGlyph(f, g);
...
void CheckGlyphs(int32_t fi, RichText* g)
{
	...
	{
		for(uint32_t* git=pit->m_text.m_data; *git; ++git)
		{
			CheckGlyph(f, *git);
		}
	}
