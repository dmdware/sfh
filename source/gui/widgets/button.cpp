...
Button::Button(Widget* parent, const char* name, const char* filepath, const RichText label, const RichText tooltip, int32_t f, int32_t style, void (*reframef)(Widget* w), void (*click)(), void (*click2)(int32_t p), void (*overf)(), void (*overf2)(int32_t p), void (*out)(), int32_t parm, void (*click3)(Widget* w)) : Widget()
{
	m_parent = parent;
	m_type = WIDGET_BUTTON;
	m_name = name;
...
void Button::draw()
{
	...
	else if(m_style == BUST_LEFTIMAGE)
	{
		...
		DrawShadowedTextF(m_font, textleft, texttop, m_crop[0], m_crop[1], m_crop[2], m_crop[3], &m_label);
	}
	else if(m_style == BUST_LINEBASED)
	{
		...
		CenterLabel(this);
		DrawShadowedTextF(m_font, m_tpos[0], m_tpos[1], m_crop[0], m_crop[1], m_crop[2], m_crop[3], &m_label);
	}
