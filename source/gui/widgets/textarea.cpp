...
TextArea::TextArea(Widget* parent, const char* n, const RichText t, int32_t f, void (*reframef)(Widget* w), float r, float g, float b, float a, void (*change)()) : Widget()
{
	m_parent = parent;
...
int32_t TextArea::rowsshown()
{
	int32_t rows = (int32_t)( (m_pos[3]-m_pos[1])/g_font[m_font].gheight );

	return rows;
}

int32_t TextArea::square()
{
...
void TextArea::inev(InEv* ie)
{
	...
	if(ie->type == INEV_MOUSEMOVE && !ie->intercepted)
	{
		if(m_ldown)
		{
			int32_t newcaret = MatchGlyphF(&m_value, m_font, g_mouse.x, m_pos[0]+m_scroll[0], m_pos[1], m_pos[0], m_pos[1], m_pos[2], m_pos[3]);
...
void TextArea::placestr(const char* str)
{
	...
	int32_t addlen = strlen(str);
	if(addlen + len >= m_maxlen)
		addlen = m_maxlen - len;

	uint32_t* addstr = ToUTF32((unsigned char*)str);
	...
	int32_t endx = EndX(&m_value, m_caret, m_font, m_pos[0]+m_scroll[0], m_pos[1]);
...
void TextArea::placechar(uint32_t k)
{
	//int32_t len = m_value.length();
	...
	int32_t currplace = 0;
	bool changed = false;
	for(std::list<RichPart>::iterator i=m_value.m_part.begin(); i!=m_value.m_part.end(); i++)
	{
		if(currplace + i->texlen() >= m_caret && !changed)
		{
			...
			if(i->m_type == RICH_TEXT)
			{
				...
				int32_t subplace = m_caret - currplace;
...
void TextArea::selectall()
{
	...
	//RichText val = drawvalue();
	int32_t endx = EndX(&m_value, m_value.texlen(), m_font, m_pos[0]+m_scroll[0], m_pos[1]);
...
void TextArea::gainfocus()
{
	if(!m_opened)
	{
		...
		r.x = (int32_t)m_pos[0];
		r.y = (int32_t)m_pos[3];
		r.w = g_width - (int32_t)m_pos[0];
		r.h = g_height - (int32_t)m_pos[3];
