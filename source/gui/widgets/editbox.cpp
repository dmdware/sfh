...
EditBox::EditBox(Widget* parent, const char* n, const RichText t, int32_t f, void (*reframef)(Widget* w), bool pw, int32_t maxl, void (*change3)(uint32_t key, uint32_t scancode, bool down, int32_t parm), void (*submitf)(), int32_t parm) : Widget()
{
	m_parent = parent;
	m_type = WIDGET_EDITBOX;
	m_name = n;
...
void EditBox::frameupd()
{
	...
	if(m_ldown)
	{
		...
		if(g_mouse.x >= m_pos[2]-5)
		{
			...
			RichText val = drawvalue();
			int32_t vallen = val.texlen();

			int32_t endx = EndX(&val, vallen, m_font, m_pos[0]+m_scroll[0], m_pos[1]);
		...
		{
			RichText val = drawvalue();
			int32_t newcaret = MatchGlyphF(&val, m_font, g_mouse.x, m_pos[0]+m_scroll[0], m_pos[1], m_pos[0], m_pos[1], m_pos[2], m_pos[3]);
...
//#endif
	...
	else if(ie->type == INEV_KEYDOWN && !ie->intercepted)
	{
		...
		if(ie->key == SDLK_LEFT)
		{
			...
			RichText val = drawvalue();
			int32_t endx = EndX(&val, m_caret, m_font, m_pos[0]+m_scroll[0], m_pos[1]);
...
#endif
		//uint32_t* ustr = ToUTF32((const unsigned char*)ie->text.c_str(), ie->text.length());
		uint32_t* ustr = ToUTF32((const unsigned char*)ie->text.c_str());
		//RichText addstr(RichPart(UStr(ustr)));	//Why does MSVS2012 not accept this?
		RichText addstr = RichText(RichPart(UStr(ustr)));
		uint32_t first = ustr[0];
		delete [] ustr;
...
{
	int32_t len = m_value.texlen();
	int32_t rawlen = m_value.rawstr().length();
	...
	int32_t addlen = str->texlen();
	int32_t rawaddlen = str->rawstr().length();
	...
	*/
	...
	uint32_t* rawaddustr = ToUTF32((unsigned char*)rawaddstr.c_str());
...
{
	uint32_t* ustr = ToUTF32( (unsigned char*)SDL_GetClipboardText() ); 
...
void EditBox::selectall()
{
	...
	RichText val = drawvalue();
	int32_t endx = EndX(&val, m_value.texlen(), m_font, m_pos[0]+m_scroll[0], m_pos[1]);
...
void EditBox::gainfocus()
{
	if(!m_opened)
	{
		...
		r.x = (int32_t)m_pos[0];
		r.y = (int32_t)m_pos[3];
		r.w = (int32_t)(g_width - m_pos[0]);
		r.h = (int32_t)(g_height - m_pos[3]);
#else
		r.x = (int32_t)g_width/2;
		r.y = (int32_t)0;
		r.w = (int32_t)g_width/2;
		r.h = (int32_t)g_height;
#endif
