...
ListBox::ListBox(Widget* parent, const char* n, int32_t f, void (*reframef)(Widget* w), void (*change)()) : Widget()
{
	m_parent = parent;
...
void ListBox::erase(int32_t which)
{
...
int32_t ListBox::rowsshown()
{
	int32_t rows = (int32_t)( (m_pos[3]-m_pos[1])/g_font[m_font].gheight );

	if(rows > (int32_t)m_options.size())
		rows = m_options.size();

	return rows;
}

int32_t ListBox::square()
{
...
void ListBox::draw()
{
	...
	if(m_selected >= 0 && m_selected >= (int32_t)m_scroll[1] && m_selected < (int32_t)m_scroll[1]+rowsshown())
	{
		glUniform4f(g_shader[SHADER_ORTHO].slot[SSLOT_COLOR], 1, 1, 1, 0.5f);
		DrawImage(g_texture[m_filledtex].texname, m_pos[0], m_pos[1]+(m_selected-(int32_t)m_scroll[1])*f->gheight, m_pos[2]-square(), m_pos[1]+(m_selected-(int32_t)m_scroll[1]+1)*f->gheight, 0,0,1,1, m_crop);
		glUniform4f(g_shader[SHADER_ORTHO].slot[SSLOT_COLOR], 1, 1, 1, 1);
	}

	for(int32_t i=(int32_t)m_scroll[1]; i<(int32_t)m_scroll[1]+rowsshown(); i++)
		DrawShadowedText(m_font, m_pos[0]+3, m_pos[1]+g_font[m_font].gheight*(i-(int32_t)m_scroll[1]), &m_options[i]);
}
...
void ListBox::inev(InEv* ie)
{
	...
	else if(ie->type == INEV_MOUSEDOWN && ie->key == MOUSE_LEFT && !ie->intercepted)
	{
		...
		for(int32_t i=(int32_t)m_scroll[1]; i<(int32_t)m_scroll[1]+rowsshown(); i++)
		{
			int32_t row = i-(int32_t)m_scroll[1];
			// std::list item?
	...
	else if(ie->type == INEV_MOUSEUP && ie->key == MOUSE_LEFT && !ie->intercepted)
	{
		...
		if(g_mouse.x >= m_pos[2]-square() && g_mouse.y >= m_pos[1] && g_mouse.x <= m_pos[2] && g_mouse.y <= m_pos[1]+square())
		{
			if(rowsshown() < (int32_t)((m_pos[3]-m_pos[1])/f->gheight))
			{
				ie->intercepted = true;
				return;
			}
