...
void AllocGrid(int32_t wx, int32_t wy)
{
	...
	if( !(g_labdem = new int32_t[ (wx*wy) ]) ) OUTOFMEM();
	if( !(g_labsup = new int32_t[ (wx*wy) ]) ) OUTOFMEM();

	memset(g_labdem, 0, sizeof(int32_t));
	memset(g_labsup, 0, sizeof(int32_t));
...
*/
void Heightmap::alloc(int32_t wx, int32_t wy)
{
	destroy();

	// Vertices aren't shared between tiles or triangles.
	int32_t numverts = wx * wy * 3 * 2;
...
void FreeGrid()
{
	for(unsigned char ctype=0; ctype<CD_TYPES; ctype++)
	{
		...
		//nice
		CdTile*& actual = ct->cdtiles[(int32_t)false];
		CdTile*& planned = ct->cdtiles[(int32_t)true];
...
void Heightmap::adjheight(int32_t x, int32_t y, signed char change)
{
	hpt[ (y)*(g_mapsz.x+1) + x ] += change;
}

void Heightmap::setheight(int32_t x, int32_t y, unsigned char height)
{
...
float Heightmap::accheight2(float x, float y)
{
	...
	int32_t tileindex = (int)(ty*m_width.x) + (int)(tx);
	int32_t tileindex6v = tileindex * 6;
...
//	Vec3f trinorm = Normal(tri);
	...
	float z = - ( x*plane.m_normal.x + y*plane.m_normal.z + plane.m_d ) / plane.m_normal.y;
...
//	Vec3f trinorm = Normal(tri);
	...
	//float z = - ( (point.x-(int)tx*TILE_SIZE)*plane.m_normal.x + (point.y-(int)ty*TILE_SIZE)*plane.m_normal.y + plane.m_d ) / plane.m_normal.z;

	float z = - ( (point.x)*plane.m_normal.x + (point.y)*plane.m_normal.y + plane.m_d ) / plane.m_normal.z;
...
#if 1
float Heightmap::accheight(int32_t x, int32_t y)
{
	...
	int32_t tileindex = ty*g_mapsz.x + tx;
	int32_t tileindex6v = tileindex * 6;
	...
	float z = - ( x*plane.m_normal.x + y*plane.m_normal.y + plane.m_d ) / plane.m_normal.z;
...
float Heightmap::accheight(int32_t x, int32_t y)
{
	...
	if(ty >= g_hmap.m_widthy)
		ty = g_hmap.m_widthy-1;
#endif
...
#if 0
	g_applog<<"- ( "<<x<<"*"<<plane.m_normal.x<<" + "<<z<<"*"<<plane.m_normal.z<<" + "<<plane.m_d<<" ) / "<<plane.m_normal.y<<" = "<<std::endl;

	g_applog<<"y = "<<y<<std::endl;
#endif

	//if(GetCd(CD_ROAD, tx, ty, false)->on)
	//	z += TILE_SIZE/30;
...
Vec3f Heightmap::getnormal(int32_t x, int32_t y)
{
...
void Heightmap::lowereven()
{
    ...
    do
    {
        ...
        for(int32_t x=0; x<g_mapsz.x; x++)
            ...
            {
                uint16_t h0 = getheight(x, y);
                uint16_t h1 = getheight(x+1, y);
                uint16_t h2 = getheight(x+1, y+1);
                uint16_t h3 = getheight(x, y+1);
                
                uint32_t minh = imin(h0, imin(h1, imin(h2, h3)));
    ...
    do
    {
        ...
        for(int32_t x=0; x<g_mapsz.x; x++)
            for(int32_t y=0; y<g_mapsz.y; y++)
            {
                ...
                uint32_t maxh = imax(h0, imax(h1, imax(h2, h3)));
...
void PackNorms(std::vector<Vec3f**> *addedvertnormals, int32_t from6v, int32_t to6v)
{
...
{
	for(int32_t x=0; x<this->m_width.x; x++)
		for(int32_t y=0; y<this->m_width.y; y++)
		{
			...
			//Log("height after "<<(uint32_t)h0);

			//Log("("<<(int32_t)h0<<","<<(int32_t)h1<<","<<(int32_t)h2<<","<<(int32_t)h3<<")");
	...
	*/
	std::vector<Vec3f**> *addedvertnormals = new std::vector<Vec3f**>[ this->m_width.x * this->m_width.y * 6 ];
	...
	*/
	...
	TileNormals *tilenormals = new TileNormals[ this->m_width.x * this->m_width.y ];
	...
	for(int32_t x=0; x<this->m_width.x; x++)
		for(int32_t y=0; y<this->m_width.y; y++)
		{
			...
			*/
			...
			int32_t tileindex6v = (y * this->m_width.x + x) * 3 * 2;
			int32_t tileindex = (y * this->m_width.x + x);
			...
			if(false)	//use 1 (next)
			{
				...
				MakePlane(&tridiv[ tileindex ].m_normal, &tridiv[ tileindex ].m_d, Vec3f(((float)x+0.5f)*TILE_SIZE, 0, ((float)y+0.5f)*TILE_SIZE), divnorm);
				...
				*/
				...
				if(x > 0)
				{
					int32_t nearbytileindex = (y * this->m_width.x + (x-1));
				...
				*/
				...
				if(y > 0)
				{
					int32_t nearbytileindex = ((y-1) * this->m_width.x + x);
					TileNormals* nearbytilenormals = &tilenormals[ nearbytileindex ];
				...
				*/
				...
				if(x-1 >= 0 && y-1 >= 0)
				{
					int32_t nearbytileindex = ((y-1) * this->m_width.x + x-1);
					TileNormals* nearbytilenormals = &tilenormals[ nearbytileindex ];
				...
				*/
				...
				// a(0) is the vertex index 0 of the two triangles.
				if(x+1 < this->m_width.x && y+1 < this->m_width.y)
				{
					int32_t nearbytileindex = ((y+1) * this->m_width.x + x+1);
					TileNormals* nearbytilenormals = &tilenormals[ nearbytileindex ];
			...
			*/
			...
			else
			{
				...
				origv[ (y * this->m_width.x + x) * 3 * 2 + 0 ] = a;
				origv[ (y * this->m_width.x + x) * 3 * 2 + 1 ] = b;
				origv[ (y * this->m_width.x + x) * 3 * 2 + 2 ] = c;
				origv[ (y * this->m_width.x + x) * 3 * 2 + 3 ] = d;
				origv[ (y * this->m_width.x + x) * 3 * 2 + 4 ] = a;
				origv[ (y * this->m_width.x + x) * 3 * 2 + 5 ] = c;
			...
			{
				texco[ (y * this->m_width.x + x) * 3 * 2 + i ].x = origv[ (y * this->m_width.x + x) * 3 * 2 + i ].x / TILE_SIZE;
				texco[ (y * this->m_width.x + x) * 3 * 2 + i ].y = origv[ (y * this->m_width.x + x) * 3 * 2 + i ].z / TILE_SIZE;
			}
			...
			norm[ (y * this->m_width.x + x) * 3 * 2 + 0 ] = norm0;
			norm[ (y * this->m_width.x + x) * 3 * 2 + 1 ] = norm0;
			norm[ (y * this->m_width.x + x) * 3 * 2 + 2 ] = norm0;
			norm[ (y * this->m_width.x + x) * 3 * 2 + 3 ] = norm1;
			norm[ (y * this->m_width.x + x) * 3 * 2 + 4 ] = norm1;
			norm[ (y * this->m_width.x + x) * 3 * 2 + 5 ] = norm1;
		}

	Vec3f* tempnormals = new Vec3f[ this->m_width.x * this->m_width.y * 3 * 2 ];
	...
	for(int32_t x=0; x<this->m_width.x; x++)
		for(int32_t y=0; y<this->m_width.y; y++)
			for(int32_t trivert = 0; trivert < 6; trivert++)
			{
				int32_t tileindex6v = (y * this->m_width.x + x) * 3 * 2 + trivert;
				int32_t tileindex = (y * this->m_width.x + x);
				...
				for(int32_t i=0; i<vertexnormals.size(); i++)
	...
	for(int32_t x=0; x<this->m_width.x; x++)
		for(int32_t y=0; y<this->m_width.y; y++)
		{
			...
			float temp = tridiv[ tileindex ].m_normal.y;
			tridiv[ tileindex ].m_normal.y = tridiv[ tileindex ].m_normal.z;
			tridiv[ tileindex ].m_normal.z = temp;
		}
#endif
#endif
