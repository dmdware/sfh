...
void RenderToShadowMap(Matrix projection, Matrix viewmat, Matrix modelmat, Vec3f focus, void (*drawscenedepthfunc)())
{
	...
	UseS(SHADER_DEPTH);
	glUniformMatrix4fv(g_shader[SHADER_DEPTH].slot[SSLOT_PROJECTION], 1, 0, g_lightproj.m_matrix);
	glUniformMatrix4fv(g_shader[SHADER_DEPTH].slot[SSLOT_MODELMAT], 1, 0, modelmat.m_matrix);
	glUniformMatrix4fv(g_shader[SHADER_DEPTH].slot[SSLOT_VIEWMAT], 1, 0, g_lightview.m_matrix);
	//glUniform4f(g_shader[SHADER_MODEL].slot[SSLOT_COLOR], 1, 1, 1, 1);
	//glEnableVertexAttribArray(g_shader[SHADER_DEPTH].slot[SSLOT_POSITION]);
	//glEnableVertexAttribArray(g_shader[SHADER_DEPTH].slot[SSLOT_TEXCOORD0]);
	...
	glUniformMatrix4fv(g_shader[SHADER_DEPTH].slot[SSLOT_MVP], 1, 0, mvp.m_matrix);
...
void RenderToDepthMap(Matrix projection, Matrix viewmat, Matrix modelmat, Vec3f focus, void (*drawscenedepthfunc)())
{
	...
	glUniformMatrix4fv(s->slot[SSLOT_PROJECTION], 1, 0, g_lightproj.m_matrix);
	glUniformMatrix4fv(s->slot[SSLOT_MODELMAT], 1, 0, modelmat.m_matrix);
	glUniformMatrix4fv(s->slot[SSLOT_VIEWMAT], 1, 0, g_lightview.m_matrix);
...
void UseShadow(int shader, Matrix projection, Matrix viewmat, Matrix modelmat, Matrix modelviewinv, float mvLightPos[3], float lightDir[3])
{
	...
	//glUniformMatrix4fvARB(s->slot[SSLOT_NORMALMAT], 1, 0, modelviewinv.m_matrix);
	//glUniformMatrix4fvARB(s->slot[SSLOT_INVMODLVIEWMAT], 1, 0, modelviewinv.m_matrix);
	...
	//glUniformMatrix4fvARB(s->slot[SSLOT_LIGHTMATRIX], 1, false, g_lightmat);
	glUniformMatrix4fv(s->slot[SSLOT_LIGHTMATRIX], 1, false, g_lightmat.m_matrix);
...
void RenderShadowedScene(Matrix projection, Matrix viewmat, Matrix modelmat, Matrix modelview, void (*drawscenefunc)(Matrix projection, Matrix viewmat, Matrix modelmat, Matrix modelviewinv, float mvLightPos[3], float lightDir[3]))
{
	...
	{
		UseS(COLOR3D);
		glUniformMatrix4fvARB(g_shader[SHADER_COLOR3D].slot[SSLOT_PROJECTION], 1, 0, projection.m_matrix);
		glUniformMatrix4fvARB(g_shader[SHADER_COLOR3D].slot[SSLOT_MODELMAT], 1, 0, modelmat.m_matrix);
		glUniformMatrix4fvARB(g_shader[SHADER_COLOR3D].slot[SSLOT_VIEWMAT], 1, 0, viewmat.m_matrix);
		glUniform4f(g_shader[SHADER_COLOR3D].slot[SSLOT_COLOR], 0, 1, 0, 1);
		glEnableVertexAttribArray(g_shader[SHADER_COLOR3D].slot[SSLOT_POSITION]);
		glEnableVertexAttribArray(g_shader[SHADER_COLOR3D].slot[SSLOT_NORMAL]);
		DrawTileSq();
	}*/
