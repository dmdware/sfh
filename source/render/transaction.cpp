...
void DrawTransx()
{
	...
	Vec3f* pos;
	//Vec4f screenpos;
	int32_t size = (int32_t)g_font[MAINFONT8].gheight;
	...
	while(tit != g_transx.end())
	{
...
#endif
#else
		cmpos = Vec3i((int32_t)pos->x, (int32_t)pos->y, (int32_t)pos->z);
		...
		x1 = (int32_t)( screenpos.x - tit->halfwidth );
		y1 = (int32_t)screenpos.y;
...
void NewTransx(Vec2i cmpos, const RichText* rtext)
{
	...
	cm3pos.z = Bilerp(&g_hmap, cm3pos.x, cm3pos.y);
	t.drawpos = cm3pos;
	t.rtext = *rtext;
