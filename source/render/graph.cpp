...
void Tally()
{
	...
	uint64_t froff = g_simframe % CYCLE_FRAMES;
	...
	//Tally average personal satiety
	{
		...
		for(int32_t i=0; i<UNITS; i++)
		{
			...
			p = ( p * (float)pcnt + (float)u->belongings[RES_RETFOOD] ) / (float)(pcnt+1);
			pcnt++;
		}
	...
	//Tally average personal funds
	{
		...
		for(int32_t i=0; i<UNITS; i++)
		{
			...
			p = ( p * (float)pcnt + (float)u->belongings[RES_DOLLARS] ) / (float)(pcnt+1);
			pcnt++;
		}
