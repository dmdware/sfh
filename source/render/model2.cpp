...
void Model2::Clear()
{
	...
	{
		if(m_Textures[i].m_diffusem)
			FreeTexture(m_Textures[i].m_diffusem);
		if(m_Textures[i].m_normalm)
			FreeTexture(m_Textures[i].m_normalm);
		if(m_Textures[i].m_ownerm)
			FreeTexture(m_Textures[i].m_ownerm);
		if(m_Textures[i].m_specularm)
			FreeTexture(m_Textures[i].m_specularm);
	}
#endif
	m_Textures.clear();
...
bool Model2::load(const char* relative, Vec3f scale, Vec3f translate, bool dontqueue)
{
	...
	for(int mi=0; mi<m_pScene->mNumMaterials; mi++)
		...
		//QueueTex(&spectex, specfile, true);
		CorrectNormals();
		}*/
						 ...
						 uint MeshIndex)
{
	...
	std::string dir = StripFile(m_relative);
	//dir = MakeRelative(dir.c_str());
	...
	for (uint i = 0 ; i < pScene->mNumMaterials ; i++)
	{
		...
		{
			QueueTex(&pmat->m_diffusem, diffrel.c_str(), false, false);
			QueueTex(&pmat->m_specularm, specrel.c_str(), false, false);
			QueueTex(&pmat->m_normalm, normrel.c_str(), false, false);
			QueueTex(&pmat->m_ownerm, teamrel.c_str(), false, false);
		}
	}
#endif
...
//				Log("Failed: %s", normrel.c_str());
		...
		if(dontqueue)
		{
			if(!CreateTex(pmat->m_diffusem, diffrel.c_str(), false, false))
				ErrMess(diffrel.c_str(), "Error loading diffuse");
			if(!CreateTex(pmat->m_specularm, specrel.c_str(), false, false))
				ErrMess(specrel.c_str(), "Error loading specular");
			if(!CreateTex(pmat->m_normalm, normrel.c_str(), false, false))
				ErrMess(normrel.c_str(), "Error loading normal");
			if(!CreateTex(pmat->m_ownerm, teamrel.c_str(), false, false))
...
//			g_applog <<"\tMaterial property: \""<<prop->mKey.data<<"\" = \""<<prop->mData<<"\"");
		...
		else
		{
			...
			char path[WF_MAX_PATH+1];
			char initdir[WF_MAX_PATH+1];
