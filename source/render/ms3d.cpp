...
void MS3DModel::loadtex(unsigned int& diffm, unsigned int& specm, unsigned int& normm, unsigned int& ownm, bool dontqueue)
{
	for ( int i = 0; i < m_numMaterials; i++ )
		...
		{
			//QueueTex(&m_pMaterials[i].m_diffusem, m_pMaterials[i].m_pTextureFilename, true);

			char basefile[WF_MAX_PATH+1];
			strcpy(basefile, m_pMaterials[i].m_pTextureFilename);
			StripPath(basefile);
			char basename[WF_MAX_PATH+1];
			sprintf(basename, "%s%s", m_relative, basefile);
			char difffile[WF_MAX_PATH+1];
			strcpy(difffile, basename);
			StripExt(basename);
			char specfile[WF_MAX_PATH+1];
			char normfile[WF_MAX_PATH+1];
			char ownfile[WF_MAX_PATH+1];
			...
			if(dontqueue)
			{
				CreateTex(diffm, difffile, false, true);
				CreateTex(specm, specfile, false, true);
				CreateTex(normm, normfile, false, true);
				CreateTex(ownm, ownfile, false, true);
			...
			{
				QueueTex(&diffm, difffile, false, true);
				QueueTex(&specm, specfile, false, true);
				QueueTex(&normm, normfile, false, true);
				QueueTex(&ownm, ownfile, false, true);
			}
		}
		//else
...
void MS3DModel::advanceanim()
{
	...
	for(int i = 0; i < m_numJoints; i++)
	{
		...
		Log(std::endl<<std::endl;
		Log("joint #"<<i<<"  of "<<pJoint->m_parent<<std::endl;
		Log("["<<m[0]<<","<<m[1]<<","<<m[2]<<","<<m[3]<<"]");
		Log("["<<m[4]<<","<<m[5]<<","<<m[6]<<","<<m[7]<<"]");
