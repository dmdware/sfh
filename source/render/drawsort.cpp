...
std::list<Depthable> g_drawlist;
std::list<Depthable*> g_subdrawq;

/*
TODO
speed boost to remove ability to have units in same collider tile,
...
Problem is it will take a int32_t time to learn a
...
{
 int32_t hash = this->Hash(inNode->mX, inNode->mY);
...
for int16_t hops between tiles.
*/
...
/*
TODO
For units that take up 2 or some
...
void VisitNode(Depthable* d, std::list<Depthable*>& drawlist, std::list<Depthable*>& drawqueue)
{
	if(d->visited)
		return;
	...
	for(std::list<Depthable*>::iterator subd=d->behind.begin(); subd!=d->behind.end(); subd++)
	{
		if(*subd == NULL)
			break;
		else
...
void DrawSort3(std::list<Depthable*>& drawlist, std::list<Depthable*>& drawqueue)
{
	...
	for(std::list<Depthable*>::iterator dit=drawlist.begin(); dit!=drawlist.end(); dit++)
	{
		Depthable* d = *dit;

		std::list<Depthable*>::iterator dit2 = drawqueue.begin();
...
void DrawSort(std::list<Depthable*>& drawlist, std::list<Depthable*>& drawqueue)
{
	...
	for(std::list<Depthable*>::iterator dit=drawlist.begin(); dit!=drawlist.end(); dit++)
	{
		...
		for(std::list<Depthable*>::iterator dit2=drawlist.begin(); dit2!=drawlist.end(); dit2++)
		{
			Depthable* d2 = *dit2;
...
bool CompareDepth2(Depthable* a, Depthable* b)
{
	//assert( a );
	//assert( b );
...
void DrawSort2(std::list<Depthable*>& drawlist, std::list<Depthable*>& drawqueue)
{
	for(std::list<Depthable*>::iterator dit=drawlist.begin(); dit!=drawlist.end(); dit++)
	{
		...
		*/
		...
		We use the min z, because otherwise all buildings
		will always be in front of units.
		*/

		int32_t nx = d->cmmax.x / PATHNODE_SIZE;
		int32_t ny = d->cmmin.y / PATHNODE_SIZE;
		int32_t nz = d->cmmin.z / PATHNODE_SIZE;
