...
void ListUniqueTexsModel(std::list<ModelHolder> &modelholders)
{
	for(std::list<Brush>::iterator mhiter = modelholders.begin(); mhiter != modelholders.end(); mhiter++)
	{
		...
		std::list<Brush>::iterator ut=uniquetexs.begin();
...
{
	for(std::list<Brush>::iterator br=map->m_brush.begin(); br!=map->m_brush.end(); br++)
	{
...
void LoadAllRGBData()
{
	...
	int i=0;
	for(std::list<Brush>::iterator ut=uniquetexs.begin(); ut!=uniquetexs.end(); ut++, i++)
	{
		char fulldiffusepath[WF_MAX_PATH+1];
...
#ifdef COMPILEB_DEBUG
		TexFitInfo* tfi = &texfitinfo[ *ut ];
		Log("tile times "<<texref[ *ut ].filepath<<" "<<tfi->tiletimes.x<<","<<tfi->tiletimes.y<<std::endl;
		...
		char fullspecularpath[WF_MAX_PATH+1];
		...
		char fullnormalpath[WF_MAX_PATH+1];
		...
		char fullownerpath[WF_MAX_PATH+1];
...
void CompileModel(const char* fullfile, EdMap* map, std::list<ModelHolder> &modelholders)
{
	...
	CreateTex(notexindex, "textures/notex.jpg", false, true);
