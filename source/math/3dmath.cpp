...
{
	float magnitude = MAG_VEC3F(vNormal);
...
Vec3f Cross(Vec3f a, Vec3f b)
{
	...
	vNormal.x = ((a.y * b.z) - (a.z * b.y));
	vNormal.y = ((a.z * b.x) - (a.x * b.z));
	vNormal.z = ((a.x * b.y) - (a.y * b.x));
...
{
	Vec3f a = Vector(tri[2], tri[0]);
	Vec3f b = Vector(tri[1], tri[0]);
...
bool InterPlane(Vec3f poly[], Vec3f line[], Vec3f *n, float *origdist)
{
...
double AngleBetweenVectors(Vec3f Vector1, Vec3f Vector2)
{
	...
	float vectorsMagnitude = MAG_VEC3F(Vector1) * MAG_VEC3F(Vector2) ;
...
Vec3f IntersectionPoint(Vec3f vNormal, Vec3f vLine[], double distance)
{
	...
	// way down the line's length.  The distance from the plane is int16_t, but the distance from
	// the actual intersect point is pretty int32_t.  If we divide the distance by the dot product
...
#undef M
	...
	/*
		Log("------------------------");
		Log("LookAt ");
		Log("eye="<<eyex<<","<<eyey<<","<<eyez<<" center="<<centerx<<","<<centery<<","<<centerz<<" up="<<upx<<","<<upy<<","<<upz);
		for(int32_t i=0; i<16; i++)
			Log("before translation["<<i<<"] = "<<m[i]);
		for(int32_t i=0; i<16; i++)
			Log("the translation["<<i<<"] = "<<mat2.m_matrix[i]);*/

	mat.postmult(mat2);
	/*
	for(int32_t i=0; i<16; i++)
		Log("final view matrix, after translation["<<i<<"] = "<<mat.m_matrix[i]);
	Log("------------------------");
...
#undef M
	...
	/*
	Log("------------------------");
	Log("set orthographic matrix ");
	Log("left="<<l<<" right="<<r<<" top="<<t<<" bottom="<<b<<" near="<<n<<" far="<<f);
	for(int32_t i=0; i<16; i++)
		Log("m["<<i<<"] = "<<m[i]);
	Log("------------------------");
	*/
...
Vec3f OnNear(int32_t x, int32_t y, int32_t width, int32_t height, Vec3f posvec, Vec3f sidevec, Vec3f upvec)
{
	...
	*/
	...
	//Log("--------------");
	//Log("onnear x,y="<<x<<","<<y<<" w,h="<<width<<","<<height<<" halfw,h="<<halfWidth<<","<<halfHeight<<" ratiox,y="<<ratiox<<","<<ratioy<<" aspect="<<aspect<<" W,Hnear="<<Wnear<<","<<Hnear);
	//Log("onnear resultx,y,z="<<result.x<<","<<result.y<<","<<result.z);
	//Log("--------------");
...
{
	int32_t count = (value + base/2.0f) / base;
