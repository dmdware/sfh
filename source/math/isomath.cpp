...
Vec2i CartToIso(Vec3i cmpos)
{
	...
	//for units and fine-grained elements that don't need to tile seamlessly
	screenpos.y -= cmpos.z *  ( (int32_t)TILE_PIXEL_RISE * 1000 / (int32_t)TILE_RISE ) / 1000;
	//screenpos.y -= CartZToIso(cmpos.z);

	return screenpos;
}

int32_t CartZToIso(int32_t cmz)
{
	...
	return ceili(cmz * TILE_PIXEL_RISE, (int32_t)TILE_RISE);
	//return cmz * ( (int32_t)TILE_PIXEL_RISE * 1000 / (int32_t)TILE_RISE ) / 1000;
	//return cmz / (int32_t)TILE_RISE * TILE_PIXEL_RISE;
}
...
int32_t CartZToIso2(int32_t cmz)
{
	//return cmy / tan(DEGTORAD(30));
	//return cmy * (TILE_PIXEL_WIDTH / 8) / (TILE_SIZE/2);
	//Log("cmy "<<cmy);
...
void CartToDepth(Vec3i cmpos, int32_t *depth)
{
	...
	//Plane3f projplane;
	//projplane.m_d = - MAG_VEC3F( Vec2i(1,1) * TILE_SIZE * MAX_MAP );
	//projplane.m_d = 0;
	//projplane.m_normal = ray;
	...
	*depth = (int32_t) Dot( ray, cmposf );	//+ (int32_t) MAG_VEC3F( Vec3f(1,1,1) * TILE_SIZE * MAX_MAP );
	...
	int32_t neardepth = (int32_t) Dot( ray, cmposf );
...
void IsoToCart(Vec2i pixpos, Vec3f *ray, Vec3f *point)
{
	...
	point2.z = (int32_t)((float)point1.z - (float)pix1.y * (float)TILE_RISE / (float)TILE_PIXEL_RISE);
