...
bool InsidePoly(Vec3f vIntersection, Vec3f Poly[], int32_t verticeCount)
{
    ...
    for (int32_t i = 0; i < verticeCount; i++)		// Go in a circle to each vertex and get the angle between
    {
...
bool InterPoly(Vec3f vPoly[], Vec3f vLine[], int32_t verticeCount, Vec3f* vIntersection)
{
    Vec3f vNormal;// = {0};
