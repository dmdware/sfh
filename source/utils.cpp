...
int FindValidPath(aiString* p_szString, const char* g_szFileName)
{
    ...
    else
    {
        ...
        char szTemp[WF_MAX_PATH*2], tmp2[WF_MAX_PATH*2];
...
void FullWritePath(const char* filename, char* full)
{
	...
	/*
	 char exepath[WF_MAX_PATH+1];
	 GetModuleFileName(NULL, exepath, WF_MAX_PATH);
