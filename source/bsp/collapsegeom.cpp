...
void Brush::colshv()
{
	...
	for(int i=0; i<m_nsides; i++)
	{
		BrushSide* a = &m_sides[i];
#if 1
		std::list<Vec3f>* sv = &a->m_sideverts;
#else
		auto sv = &sideverts[i];
#endif
...
void SideCorners(Winding *wind, Vec3f vmin, Vec3f vmax, Vec3f quadaxis1, Vec3f quadaxis2, BrushSide* s)
{
	...
	float fored = DOT_VEC3F(forenorm - s->m_plane.m_normal);
	float backd = DOT_VEC3F(backnorm - s->m_plane.m_normal);
...
void SideCorner3(Vec3f &corner, Plane3f* pl, Vec3f bound)
{
	...
	for(int i=0; i<3; i++)
		binter[i] = LineInterPlane(l[i], pl->m_normal, pl->m_d, &interp[i]);
...
{
	for(std::list<Vec3f>::iterator viter=s->m_sideverts.begin(); viter!=s->m_sideverts.end(); viter++)
	{
	}
}