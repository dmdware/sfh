...
void CheckBrush(TraceWork* tw, TraceJob* tj, Vec3f start, Vec3f end, float* traceratio, Vec3f* clip, Brush* b)
{
	...
	for(int i = 0; i < b->m_nsides; i++)
	{
		...
		float startdistance = Dot(start, p->m_normal) + (p->m_d + offset);
		float enddistance = Dot(end, p->m_normal) + (p->m_d + offset);

		//float startdistance0 = Dot(start, p->m_normal) + (p->m_d);
		//float enddistance0 = Dot(end, p->m_normal) + (p->m_d);
		...
		if(tj->type == TJ_TRACE_BOX)
		{
			...
			startdistance = Dot(start + voffset, p->m_normal) + p->m_d;
			enddistance = Dot(end + voffset, p->m_normal) + p->m_d;
		}
		...
		if(startdistance > enddistance)	//closer to plane inside - advance startratio if more positive
		{
			...
			if(ratio1 > startratio)
			{
				...
				collisionnormal = p->m_normal;
				
#ifdef BCOLDEBUG
				colbs = s;
#endif

				if((start.x != end.x || start.z != end.z) /* && p->m_normal.y != 1 */ && p->m_normal.y >= 0.0f)
				...
				//tw->trytostep = trytostep ? true : tw->trytostep;
...
#if 0
			g_order.push_back(OrderMarker(colbs->m_centroid, GetTicks(), 100));
...
reccol:
	...
	for(int i = 0; i < b->m_nsides; i++)
	{
		...
		g_applog<<"\t\t plane: "<<p->m_normal.x<<","<<p->m_normal.y<<","<<p->m_normal.z<<":"<<p->m_d<<std::endl;
