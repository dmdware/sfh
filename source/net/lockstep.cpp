...
uint32_t TurnFrames()
{
	...
	*/
	...
	//if the wait is too int32_t, the workers die
	//else if(fast)
	//	return FASTTURN;
...
bool CanStep()
{
	...
	uint32_t turnframes = TurnFrames();
	...
	*/
	...
	else if(g_netmode == NETM_HOST)
	{
		...
		pay to demolish buildings, labour
		*/
			...
			//for(int32_t pi=0; pi<1; pi++)
		{
			...
			if( (c->curnetfr / turnframes) != ((g_netframe + 1) / turnframes) &&
				(c->curnetfr / turnframes) != ((g_netframe + 1) / turnframes) )
			{
#if 0
				char msg[128];
...
int32_t LockPackSz(int32_t packtype, PacketHeader* pack)
{
	int32_t sz = 0;
	...
	*/
	...
	case PACKET_MOVEORDER:	//variable length!
		sz = sizeof(MoveOrderPacket) + 
			sizeof(uint16_t) * ((MoveOrderPacket*)pack)->nunits;
		return sz;
...
void AppendCmds(std::list<PacketHeader*>* q, NetTurnPacket* ntp)
{
	...
	while( (int32_t)(pload-pstart) < ntp->loadsz )
	{
		...
		int32_t sz = LockPackSz(ph->type, ph);
...
void FillNetTurnPacket(NetTurnPacket** pp, std::list<PacketHeader*>* q)
{
	...
	*pp = (NetTurnPacket*)malloc(sizeof(NetTurnPacket)+loadsz);
	...
	for(std::list<PacketHeader*>::iterator pi=q->begin(); pi!=q->end(); pi++)
	{
		int32_t packsz = LockPackSz((*pi)->type, *pi);
...
void Sv_StepTurn()
{
	...
	SendAll((char*)ntp, sizeof(NetTurnPacket) + ntp->loadsz, true, false, NULL, 0);
...
void AppendCmd(PacketHeader* p, int32_t sz, std::list<PacketHeader*>* q)
{
...
{
	int16_t sz = LockPackSz(pack->type, pack);
