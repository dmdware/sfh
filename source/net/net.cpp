...
//uint64_t g_lastS;  //last sent
//uint64_t g_lastR;  //last recieved

#ifndef MATCHMAKER
int32_t g_netmode = NETM_SINGLE;
#else
int32_t g_netmode = NETM_HOST;
uint32_t g_transmitted = 0;
#endif
...
void CheckAddSv()
{
	//check sv hosts for info
#if 0
	for(std::list<Widget*>::iterator cit=g_conn.begin(); cit!=g_conn.end(); cit++)
	{
		NetConn* nc = &*cit;
	...
	if(v)
	{
		std::list<SvInfo>::iterator sit = v->m_svlist.begin();
...
void SendSvs()
{
	...
	//
		...
		std::list<Widget*>::iterator ait = g_conn.begin();	//address iterator
		int32_t ain = 0;	//address index
	...
	uint32_t svipaddr = SDL_SwapBE32(ait->addr.host);
	uint16_t svport = SDL_SwapBE16(ait->addr.port);
	uint32_t svipaddr2 = SDL_SwapBE32(ci->addr.host);
	uint16_t svport2 = SDL_SwapBE16(ci->addr.port);
...
void CheckGetSvs()
{
	...
	const int32_t maxconn = imax(NETCONN_TIMEOUT/RESEND_DELAY, 50);
	int32_t numconn = 0;
	uint64_t latest = 0;

	for(std::list<NetConn>::iterator sit=g_conn.begin(); sit!=g_conn.end(); sit++)
	{
		if(!sit->ishostinfo)
			continue;
...
void EndConns()
{
	std::list<NetConn>::iterator cit = g_conn.begin();
...
void UpdNet()
{
	...
	//uint32_t svipaddr = SDL_SwapBE32(addr->host);
	//uint16_t svport = SDL_SwapBE16(addr->port);
...
//#ifdef MATCHMAKER
			...
			{
				char msg[128];
				sprintf(msg, "recv ack%u t%d", (uint32_t)((PacketHeader*)in->data)->ack, ((PacketHeader*)in->data)->type);
				InfoMess("recv", msg);
			}
#endif
...
#if 1
			unsigned char *in2;
			int32_t bytes2 = Unparify((unsigned char*)in->data, &in2, bytes);
			if(!bytes2)
			{
	...
	static uint64_t lastrenew = GetTicks();
...
#if 1
	static uint64_t lastlog = GetTicks();
