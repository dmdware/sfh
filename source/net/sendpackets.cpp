...
void SendData(char* data, int32_t size, IPaddress * paddr, bool reliable, bool expires, NetConn* nc, UDPsocket* sock, int32_t msdelay, void (*onackfunc)(OldPacket* p, NetConn* nc))
{
	...
	{
		char msg[128];
		sprintf(msg, "send netturn for netfr%u load=%u", ((NetTurnPacket*)data)->fornetfr, (uint32_t)((NetTurnPacket*)data)->loadsz);
		//InfoMess("s", msg);
		Log(msg);
...
#if 1
	unsigned char *data2;
	int32_t size2 = Parify((unsigned char*)data, &data2, size);
	...
	int32_t outsz = Unparify(data2, &tempout, size2);
	...
	Log("send t"<<((PacketHeader*)data)->type);
			...
			char msg[128];
			sprintf(msg, "send ack%u t%d", (uint32_t)((PacketHeader*)out->data)->ack, ((PacketHeader*)out->data)->type);
			InfoMess("send", msg);
#endif
...
void ResendPacks()
{
	...
	//uint64_t due = now - RESEND_DELAY;
	//uint64_t expire = now - RESEND_EXPIRE;
	...
	while(i!=g_outgo.end())
	{
        ...
        //which would cause an overflow for uint64_t.
        uint64_t safelast = enmin(p->last, now);
        uint64_t passed = now - safelast;
        uint64_t safefirst = enmin(p->first, now);
	...
	while(i!=g_outgo.end())
	{
		...
		uint32_t nextdelay = RESEND_DELAY;
        uint64_t firstpassed = now - safefirst;

		if(nc && firstpassed >= RESEND_DELAY)
		{
			//fixed
			uint64_t sincelast = safelast - safefirst;
			//old: 30, 60, 120, 240, +?
		...
		//if(p->last > due)
		//if((nc && passed > (uint32_t)(0.8f * nc->ping)) || (!nc && passed > RESEND_DELAY))
		...
		*/
		...
		if(nc)
		{
			uint16_t lastack = nc->nextsendack - SLIDING_WIN - 1;
