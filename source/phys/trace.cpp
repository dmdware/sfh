...
bool PassUnits(Vec2i vstart, Vec2i vend,
               int32_t cmminx, int32_t cmminy, int32_t cmmaxx, int32_t cmmaxy,
               Unit* u2)
{
	...
	int32_t cmminx2 = u2->cmpos.x - u2t->size.x/2;
	int32_t cmminy2 = u2->cmpos.y - u2t->size.x/2;
	int32_t cmmaxx2 = cmminx2 + u2t->size.x;
	int32_t cmmaxy2 = cmminy2 + u2t->size.x;
	...
	for(int32_t i = 0; i < 4; i++)
	{
...
#endif
		{
			...
			voffset.y = (p->m_normal.y < 0) ? cmmaxy : cmminy;
#if 0
          ...
          Unit* thisu, Unit* targu, Building* targb)
{
	...
	int32_t cmminx = -ut->size.x/2;
	int32_t cmminy = -ut->size.x/2;
	int32_t cmmaxx = cmminx + ut->size.x;
	int32_t cmmaxy = cmminx + ut->size.x;
	...
	int32_t nminx = absmin.x / PATHNODE_SIZE;
	int32_t nminy = absmin.y / PATHNODE_SIZE;
	int32_t nmaxx = absmax.x / PATHNODE_SIZE;
	int32_t nmaxy = absmax.y / PATHNODE_SIZE;
